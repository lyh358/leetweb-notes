...
在编译期，模板实例化阶段，对于某个消息类型T，T在尝试使用实际类型代替偏特化模板的形参的过程中（此时没有进入结构体内部代码），如果产生了非法类型/表达式，此时编译器不报编译错误，而是把这个模板特化版本从重载候选集里丢弃，编译器继续去找其它可行版本。
...
全特化没有 SFINAE 参与，是 “类型完全对上就用，对不上就走主模板”。

但如果你不想每一个 protobuf 消息都手写一遍全特化，想写一个**偏特化，匹配所有 `google::protobuf::Message` 的派生类**，这时就必须 SFINAE。
