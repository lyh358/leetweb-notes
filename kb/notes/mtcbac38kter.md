...
=={yellow}ROS==**=={yellow}不是==**=={yellow}类似==**=={yellow}Linux==**=={yellow}或==**Windows**=={yellow}的===={yellow}**传统操作系统**===={yellow}，而是==**=={yellow}运行在Linux之上==**=={yellow}的==**机器人软件框架**=={yellow}。==
...
1. =={yellow}**同一个数据结构**需要**维护**==`.proto`=={yellow}和`.msg`==**两份定义**；
...
=={yellow}Protobuf，也就是==**=={yellow}Protocol Buffers==**=={yellow}，是一种===={yellow}Google 开源，==**=={yellow}跨语言、跨平台==**=={yellow}的==**结构化数据定义**=={yellow}=={yellow}和===={yellow}**二进制序列化**===={yellow}框架===={yellow}。==
...
**=={pink}使用流程：==**
**1）**=={green}开发者==**=={green}使用 .proto 文件定义消息==**=={green}：==
...
> =={green}ROS==负责机器人**节点的组织**和**发布订阅**，=={green}Protobuf==负责提供**=={yellow}更通用的数据契约==**和**=={yellow}更强的==****=={yellow}序列化能力==**。
...
- ROS业务通信链路负责感知、规划、控制等功能=={yellow}节点之间的数据交换==；
- 性能监控链路负责=={yellow}采集和展示==各个Linux计算=={yellow}节点的运行状态==。
