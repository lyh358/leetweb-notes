...
=={yellow}图像传输==**=={yellow}数据量大，占用的CPU，内存，网络资源多==**=={yellow}，和传感器功能==**=={yellow}耦合==**=={yellow}在一起会==**=={yellow}占用太多资源，===={yellow}降低系统稳定性==**=={yellow}，同时分离也便于==**=={yellow}单独调试==**=={yellow}和**故障隔离**==。
...
- =={yellow}增加**太阳能电池组，增加续航**电源保护==、稳压和电池管理；
...
=={yellow}Arduino框架并没有绕开FreeRTOS==。=={green}**系统启动后，Arduino会创建**==`loopTask`=={green}并在其中调用==`setup()`=={green}和`loop()`。项目把**主要业务拆成六个独立FreeRTOS任务**，因此==`loop()`**只保留周期休眠**，不再承载业务逻辑。
...
=={green}**esp32没有MPU**（内存保护单元）**任务全部跑在特权状态，通过中断上下文进行系统内核操作；**但是**esp32s3有MPU**，**支持特权级**操作，可以创建**硬件隔离的用户态任务**。==
如果想要隔离不可信代码，ESP32 不能靠 FreeRTOS 特权机制，需要用=={green}**双核隔离（一个核跑可信代码，另一个跑不可信，通过队列通信**）==或者换 S3。
...
| 任务 | 优先级 | 主要职责 |
| --- | --- | --- |
| `taskNetManager` | =={yellow}4== | =={yellow}**Wi-Fi/MQTT连接维护**、**心跳**、**重连**==、**=={yellow}ArduinoOTA==**和**=={yellow}URL OTA==** |
...
| 任务 | 优先级 | 主要职责 |
| --- | --- | --- |
| `taskSensorAcquire` | =={yellow}3== | =={yellow}**DHT22**和**HX711  长****周期采集**==、=={yellow}**告警位生成**和**消息入队**== |
...
- =={green}**判断MQTT**是否**可用**；==
- =={green}从**遥测队列**中**读取**新数据**组装AlinkJSON**格式并**发布**==；
- =={green}**发布失败**时将数据写入**环形缓存**；==
- =={green}**网络恢复**后**限速补发离线数据**；==
- =={green}**周期上报**设备**状态**。==

=={pink}网络恢复==后每轮最多=={pink}补发==4条，=={pink}**避免历史数据长期占用链路**==。

### =={yellow}4.3 HTTP服务任务（2）==
...
### =={yellow}4.6 OLED显示任务（1）==

`taskOledDisplay`负责：

- =={green}**从共享内存（全局z't'b'l）复制系统最新状态**；==
- =={green}获取**I²C互斥锁**；==
- =={green}**绘制页面：**温湿度、重量、Wi-Fi、MQTT、GPS和缓存状态==；
- =={green}**一次性发送OLED缓冲区**==；
- =={green}**释放I²C锁**并**休眠**。==
...
## =={pink}5. 任务之间如何通信与同步（5项）（queue+全局变量+环形缓冲区+EventGroup+Mutex）==
...
=={green}当Wi-Fi断开时，同时清除Wi-Fi和MQTT状态==，因为MQTT依赖底层网络连接。
...
### =={yellow}5.4 全局变量SharedState：保存最新状态==
...
```undefined
获取锁 → 复制完整快照 → 归还锁 → 数据使用和处理
```

=={green}**不会拿着锁进行**JSON拼接、网页生成或OLED绘图等**处理**，**避免临界区过长**==。

### =={yellow}5.5 RAM环形缓存：短时断网缓冲==
...
=={green}网络恢复后，由**上传任务按顺序补发**。缓存满时**覆盖最旧数据**，以保留最新状态。==

=={green}它**属于共享数据结构**，因此由==`g_cacheMutex`=={green}保护。==
...
> **=={yellow}基于FreeRTOS的双核、抢占式、软实时物联网终端==。**
