...
**=={green}替换逻辑==**=={green}：Erf是PyTorch== `F.gelu()`=={green}的默认实现，数学上更精确，MACs减少55%==。预期Vector MACs降低30%，理论总时间从152.86μs降至~110μs（-28%）。
...
=={green}**理论上**4层Transformer共**减少12个计算节点 + 8个常量节点，55%MACs计算量，模型纯计算开销降低28%（）152**μs**-110us**。==
