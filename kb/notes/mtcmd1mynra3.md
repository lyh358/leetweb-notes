...
4. **MonitorAgent**：**=={yellow}采集器对象的唯一拥有者==**。Agent 内部`vector<unique_ptr<IMetricCollector>> collectors`=={yellow}**接管全部采集器的所有权**==；=={yellow}Agent 析构的时候，vector 销毁，全部采集器自动释放，无内存泄漏。==
