...
在=={green}不同的充放电循环==中，电压、电流、温度、充放电时间等=={green}数据都会发生变化==，而且=={green}不同特征==对电池=={green}老化的敏感程度==并不相同。部分特征与容量衰减关系=={green}较强==，部分特征可能包含冗余或噪声。如果直接把所有监测数据送入预测模型，不但会增加模型复杂度，还可能干扰模型对真实退化趋势的学习。
