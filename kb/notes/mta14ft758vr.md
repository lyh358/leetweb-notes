...
> 这个项目不是把ROS消息“转换成”Protobuf，也不是因为Protobuf更流行就把ROS消息替换掉，而是**=={yellow}让ROS通信框架同时兼容ROS Msg和Protobuf两类消息==**，从而=={yellow}**保留ROS生态**==，=={yellow}同时接入更通用的Protobuf数据生态==。
...
这样拆分以后，系统更容易开发和维护，但也带来了一个问题：

> 不同程序之间怎样交换数据？

例如，感知模块识别出一个障碍物后，需要把障碍物的位置、速度和类别发送给规划模块。规划模块计算出轨迹后，又要把轨迹发送给控制模块。

因此，自动驾驶机器人需要一个负责组织软件模块并帮助它们通信的中间件，ROS就是在这个位置出现的。

## 2. ROS在机器人系统中扮演什么角色

ROS不是类似Linux或Windows的传统操作系统，而是运行在Linux之上的机器人软件框架。
...
ROS提供了多种通信方式，其中最常用的是Topic发布订阅：

- 发送数据的节点是发布者；
- 接收数据的节点是订阅者；
- 双方通过同一个Topic交换数据。

发布者不需要知道具体有哪些订阅者，订阅者也不需要直接调用发布者，因此各个模块能够保持松耦合。
...
ROS原生通过`.msg`文件描述这种数据结构，这套消息体系通常称为ROS Msg或rosmsg。
...
- `.msg`是开发阶段使用的消息定义文件；
- ROS在网络上传输的并不是`.msg`文本，而是序列化后的二进制数据。
...
这些系统不一定运行在ROS环境中。如果业务数据原本已经使用`.proto`定义，而接入ROS时又必须重新定义一份`.msg`，就会出现两套数据模型。
...
1. 同一个数据结构需要维护`.proto`和`.msg`两份定义；
2. 需要为不同消息编写重复的转换代码；
3. 字段修改时，两套定义和转换逻辑都要同步更新。

消息数量较少时影响不明显，但自动驾驶系统中的消息类型很多，长期维护成本会逐渐增加。

## 5. Protobuf是什么

Protobuf，也就是Protocol Buffers，是一种通用的结构化数据定义和二进制序列化机制。
...
然后通过`protoc`生成C++、Python、Java等语言对应的代码。

它的价值主要体现在四个方面：

- **跨语言**：同一份`.proto`可以生成多种语言代码；
- **跨系统**：不依赖ROS，可以用于gRPC、服务端和其他程序；
- **结构紧凑**：采用字段编号、Varint等二进制编码方式；
- **便于演进**：在遵守字段编号规则的情况下，可以新增字段，并保持一定的前后版本兼容性。

Protobuf确实被广泛用于分布式服务、云边通信和部分自动驾驶平台，但不能简单说“Protobuf一定比ROS Msg更好”。
...
> 希望同一份消息定义既能用于ROS节点通信，也能复用于gRPC和其他非ROS系统，减少重复定义和转换代码。

至于序列化速度和数据体积，Protobuf在部分数据结构上可能更有优势，但本项目没有进行严格的性能基准测试，因此面试中不声称具体提升比例。

## 6. 为什么Protobuf不能直接放进ROS

虽然Protobuf能够生成C++消息类，但ROS默认并不认识这种类型。

ROS在允许一种类型通过Topic传输之前，需要知道：
...
> 扩展ROS的类型识别和序列化机制，让ROS知道怎样识别、打包和还原Protobuf消息。
...
上层节点仍然使用ROS熟悉的发布订阅接口，不需要重新设计通信框架。

### 减少两套消息之间的转换

已有Protobuf消息可以直接进入ROS通信链路，减少重复的`.msg`定义和转换代码。

### 便于接入非ROS系统

同一份`.proto`可以继续用于gRPC、云端服务或其他语言编写的程序。

### 改善消息的长期演进能力

Protobuf使用字段编号描述数据。只要遵守不随意修改旧字段编号等规则，就可以更平滑地增加新字段。

因此，这个项目的价值可以概括为：

> ROS负责机器人节点的组织和发布订阅，Protobuf负责提供更通用的数据契约和序列化能力。

## 8. 为什么继续引出分布式监控

解决了业务消息通信后，系统还有另一个问题。

自动驾驶机器人通常包含多个计算密集型模块。感知算法可能持续占用CPU，规划模块需要稳定的内存和计算资源，多个节点之间还需要通过网络传输数据。

如果这些模块分布在多个Linux计算节点上，仅仅知道ROS Topic是否能够通信还不够，还需要知道：
...
针对ROS原生消息难以直接复用Protobuf数据结构，以及分布式Linux计算节点缺少集中性能监控的问题，我计划搭建一套面向自动驾驶机器人的**分布式通信与监控系统原型**。

整个项目需要完成两项核心任务：

1. 扩展ROS的消息类型识别和序列化机制，使ROS Topic在保留原生ROS Msg通信能力的同时，也能够直接传输Protobuf消息；
2. 构建Linux节点性能采集与集中监控链路，将多个节点的CPU、内存和网络等指标汇总到中心端进行展示。
...
监控侧的目标，是将分散在多个Linux计算节点上的性能信息统一汇总到中心端。
...
不是只针对一个具体类型，而是针对满足某种形式或条件的一类类型。

这个项目面对的不是某一个固定的Protobuf消息，而是所有继承自`google::protobuf::Message`的类型，因此更适合使用模板偏特化。
...
这样，无论以后增加`PublishInfo`、`NodeMetrics`还是其他Protobuf消息，都不需要为每种类型重新编写一套ROS适配代码。
