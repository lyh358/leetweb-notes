...
=={yellow}但是GRA只能剔除弱相关特征，不能解决入选==**=={yellow}特征之间的信息冗余==**=={yellow}，因此我又使用==**=={yellow}PCA进行降维==**=={yellow}。==********==******=={yellow}****={yellow}前四个主成分累计方差贡献率达到98.65%****==**{yellow}****=={yellow}，所以最终将9维特征==**=={yellow}压缩成4维健康因子==**。

=={yellow}之后，我==**=={yellow}把连续多个循环的健康因子输入LSTM==**=={yellow}，通过LSTM学习电池长期、非线性的容量退化规律，再==**=={yellow}通过全连接层输出容量预测值==**=={yellow}。模型不是直接输出RUL，而是==**=={yellow}先得到未来容量曲线==**=={yellow}，==********==******=={yellow}****={yellow}再根据EOL阈值找到容量首次越过阈值的循环位置**=={yellow}****=={yellow}，==****==**={yellow}进而计算剩余寿命**=={yellow}****=={yellow}。==
...
=={green}如果不做处理，**每次调用模型**都会从零状态开始，跨次推理的时序上下文可能丢失。==
