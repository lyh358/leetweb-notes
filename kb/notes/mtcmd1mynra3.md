...
1. =={yellow}写==`DiskCollector`===={yellow}={yellow}**继承**=={yellow}`IMetricCollector`=={yellow}，实现 Collect ()==
...
- =={yellow}节点能够持续上报指标，而不是每次重新建立请求==；
- =={yellow}中心能够持续向监控端推送最新数据。==

=={yellow}因此==，项目选择了gRPC的流式通信。
