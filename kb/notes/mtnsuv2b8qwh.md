...
- =={green}缺点==：**违反开闭原则**。**新增产品，就要修改工厂内部的 if‑else/switch 代码**；工厂类职责过重。
- 适用：产品种类少，产品变化不大的场景。

### =={pink}2. 抽象工厂==

**生产产品族，一组相关联的产品**。
1. 有抽象工厂基类，定义一组生产接口；每个具体工厂实现这一组接口。
2. 每一个具体工厂，可以产出一套配套的多个产品。
3. 新增一套产品族：新增一个具体工厂类即可，不用修改旧代码，符合开闭；**但新增产品等级（增加新产品接口）要修改抽象工厂，改动大**
