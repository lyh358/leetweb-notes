...
> =={yellow}**多个Agent**可能同时更新**节点状态**，**订阅端**也可能同时读取==。如果不加锁，会产生数据竞争，所以使用互斥锁保护共享的节点快照表。
...
> 本项目 Qt**不是业务机器人 UI，=={yellow}是监控客户端 GUI==**=={yellow}。==

### 在项目中的角色

完整监控链路：Agent 采集 → gRPC 上报 Center → **Qt 界面作为 gRPC 客户端去订阅 Center，展示监控数据**。

工作流程：

1. Qt 程序本身是一个**gRPC 客户端**，调用`SubscribeMetrics`服务端流接口，向 Monitor Center 发起订阅请求。
...
> 所以收到监控数据后，通过 Qt 信号槽把数据抛到 UI 主线程再渲染界面，避免崩溃。

### CMake 中和 Qt 的配合

CMake 开启 Qt 的 MOC 元对象编译，处理信号槽；链接 Qt Widgets 模块，编译出 GUI 可执行程序`qt_monitor_gui`。

### 面试口述

> Qt 实现监控系统的可视化客户端。Qt 程序作为 gRPC 客户端，调用服务端流接口订阅 Monitor Center 的数据。gRPC 网络接收在子线程完成，通过 Qt 信号槽把监控数据投递到 UI 主线程，界面展示各个计算节点 CPU、内存、网络指标以及节点健康状态，完成监控数据可视化。
