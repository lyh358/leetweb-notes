...
1. =={yellow}阅读**系统**与软件**需求**；==
2. =={yellow}明确**检测对象**、**执行条件**和**安全目标**；==
3. =={yellow}确定数据来源==以及ASW、CDD、FPGA的职责边界；
4. 实现=={yellow}初始化、周期检测和状态判断==；
5. 设计=={yellow}故障确认、去抖和恢复策略==；
6. =={yellow}接入DTC、DEM及冻结帧信息==；
7. =={yellow}增加故障注入接口==；
8. 通过日志、=={yellow}上位机和实机验证==故障上报及恢复链路。
