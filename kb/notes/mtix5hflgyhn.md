...
| 部分 | 含义 | 主要职责 |
| --- | --- | --- |
| **=={green}Host==** | 运行Linux的CPU侧 | 加载OM、准备输入、申请内存、发起推理并获取结果 |
| **=={green}Device==** | Ascend 310P3 NPU侧 | 执行模型中的矩阵乘、Softmax和数据搬运等操作 |
...
这里不仅是把代码写进同一个函数，更关键的是：

> 多个计算阶段进入同一个执行上下文，可以共享L1、L0和UB中的片上数据。
...
1. 减少Kernel启动和调度；
2. 避免Score、Weight反复写回GM；
3. 跨越ATC无法自动融合的Softmax边界。
...
> 这个算子叫什么、接收什么输入、产生什么输出，以及支持什么Shape、数据类型和数据格式。
...
- 不再单独启动TransData Kernel，而是在数据搬运或现有计算过程中顺便完成格式调整。

### 6.3 具体实现

#### 6.3.1 输入端：通过算子接口声明NZ格式
...
这样 Kernel内部不需要先启动三个独立的 ND→NZ转换过程，而是直接按照 NZ地址读取 Q、K、V。
