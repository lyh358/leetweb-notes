...
| 外设 | 硬件接口 | 数据特点 | 主要用途 |
| --- | --- | --- | --- |
| DHT22 | =={yellow}专用==**=={yellow}单总线GPIO==** | 低频、时序敏感 | 温湿度监测 |
| HX711 | **=={yellow}DATA＋SCK 双线GPIO时序==** | 低速、高精度采样 | 蜂箱重量监测 |
| NEO-6M | **=={yellow}UART==** | 连续NMEA字节流 | 定位、导航和防盗 |
| SSD1306 OLED | **=={yellow}I²C==** | 低频显示输出 | 本地状态展示 |
| OV2640 | **=={yellow}DVP＋I²S/DMA==** | 大数据量图像帧 | 活跃度与胡蜂识别 |
...
> =={yellow}ESP32**拉低数据线**发送**起始信号**==
>         ↓
> =={yellow}DHT22**返回响应信号**==
>         ↓
> =={yellow}**传输40位数据**==
>         ↓
> =={yellow}**湿度16位**＋**温度16位**＋**校验8位**==
...
5.3 UART DMA 接收（ESP32 高性能方案）GPS 字节流入 UART 硬件 FIFO，DMA 硬件自动把字节搬运到内存缓冲区，**不需要 CPU 参与搬运**。DMA 完成 / 半满 / 超时，才产生一次中断通知 CPU。CPU 一次拿到一大段 GPS 数据。

### ESP32 UART 支持 DMA，不需要 CPU 搬运字节。优势：大量串口数据流的时候，大大减少中断次数，减轻 CPU 负担，适合 GPS 这种持续输出数据流。 ESP‑IDF uart_driver_install() 底层可以开启 DMA。
...
=={yellow}为什么用 I²S 接摄像头==

| 原因 | 说明 |
| --- | --- |
| **=={yellow}ESP32 没有专用 DVP接口==** | 但 I²S 的并行数据接收能力可以复用 |
| **=={yellow}I²S 支持 DMA==** | 数据自动进内存，CPU 零拷贝，省资源 |
...
> 这里应准确表述为：我通过`esp_camera`配置和使用底层I²S/DMA采集能力，而不是从零手写DMA驱动。
