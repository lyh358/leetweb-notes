...
1. 参数是否加载完成；
2. Flash参数头部保存的 `storageCrc`；
3. 参数加载过程中实时计算的 `calcCrc`。
...
- `0`通常表示尚未初始化或没有获取到有效结果；
- `0xFFFFFFFF`是读取失败后主动填入的哨兵值。
...
> CRC读取失败并不会被当成CRC不一致故障，而是屏蔽本次故障上报，避免底层接口异常被误判成参数损坏。
...
1. 查询是否存在产线故障注入；
2. 没有注入时检查双CRC是否有效并进行比较；
3. 将最终Pass/Fail结果上报DEM。
...
只要任意一个CRC无效，就不执行一致性判断，并且当前CRC故障不报错。
...
当前功能的防抖次数为1，因此一次有效的CRC不一致即可确认故障。
