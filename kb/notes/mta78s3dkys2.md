...
| =={yellow}精度== | **=={yellow}余弦相似度==** | **优化模型**与**基准模型 输出方向**和**=={yellow}整体分布===={yellow}**的==**===={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}一致性==** |
...
### 维度值含义dim01Batch size，固定为1（单条推理）dim14天线端口数 / RB组数（10RB 配置下4个 port）dim232时域符号数（OFDM 符号数 / SRS 序列长度）dim332频域子载波数（10RB 对应的子载波维度）dim41实/虚部通道（1=仅实部；复数信号拆成2通道时为2）
