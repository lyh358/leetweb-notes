...
| 概念                  | 一句话                              || :------------------ | :------------------------------- || **Thread mode**     | 正常执行用户代码/RTOS任务的状态               || **Handler mode**    | 进入中断/异常服务程序时的状态，固定 Privileged    || **Privileged**      | 最高权限，可访问所有寄存器和内存                 || **Unprivileged**    | 受限权限，不能访问 NVIC/MPU 等核心外设，某些内存被禁止 || **MPU**             | 硬件内存保护单元，给不同区域设置读/写/执行权限         || **MemManage Fault** | 违反 MPU 规则（如越权写只读区）时触发的异常         || **HardFault**       | 最高优先级异常，所有其他异常无法处理或配置错误时的兜底      |
