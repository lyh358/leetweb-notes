...
项目定位：这是一个基于**昇腾边缘计算平台**的**锂电池寿命预测系统**

## 1. 行业背景：为什么需要预测锂电池剩余寿命

锂电池被=={yellow}广泛应用在**新能源汽车、储能系统和航天设备**==中。随着=={green}**充放电循环次数增加**==，电池内部会发生材料老化和电化学副反应，表现为=={green}**容量逐渐下降**、**内阻上升**以及**可用性能衰减**==。

对于使用锂电池的系统来说，=={yellow}只知道电池当前**还有多少电是不够的**==。SOC反映的是电池当前的剩余电量，而SOH和RUL关注的是电池已经=={green}**老化**到什么**程度**==，以及=={green}距离**寿命终**点**还剩多少**个充放电**循环**==。

如果不能提前判断剩余寿命，维护人员通常只能按照固定周期更换电池，或者等到容量明显下降以后再处理。前者可能造成电池尚未失效就被提前更换，增加维护成本；后者则可能导致设备性能下降，甚至增加系统运行风险。因此，提前预测电池的容量退化趋势和剩余寿命，可以为=={yellow}**电池维护**、**更换计划**和**健康管理**==提供依据。

## 2. 技术背景：为什么这个问题比较难

锂电池退化并=={yellow}**不是**一个简单的**线性过程**==。

在**不同的充放电循环**中，电压、电流、温度、充放电时间等数据都会发生变化，而且不同特征对电池老化的**敏感程度**并不相同。部分特征与容量衰减关系较强，部分特征可能包含冗余或噪声。如果直接把所有监测数据送入预测模型，不但会增加模型复杂度，还可能干扰模型对真实退化趋势的学习。

另外，电池容量曲线可能出现**局部波动**和**短期容量回升**现象。特别是在电池寿命的中后期，也就是接近EOL的阶段，容量变化对寿命终点判断更加敏感。即使预测值只有较小偏差，也可能导致预测的EOL循环位置发生明显变化。

因此，这个项目需要解决的不只是“能不能拟合容量曲线”，更重要的是：

- 如何从**多维**充放电数据中找到**真正**能够**反映电池退化**的特征；
- 如何减少**特征之间**的**冗余**和**噪声**；
- 如何学习电池**长期、非线性**的**退化规律**；
- 如何提高寿命中后期，尤其是**EOL附近**的**预测可靠性**。

---------------------------------------------------------------------------------------------------------------------这个问题的**难点**在于

- =={yellow}电池退化是一个==**=={yellow}长期、非线性的时序过程==**
- 而且=={yellow}原始监测数据里==有很多电压、电流、温度等变量（11维度数据），**=={yellow}不是所有特征都对寿命变化同样敏感==**=={yellow}。==
- =={yellow}如果直接把==**=={yellow}所有特征都送进模型==**=={yellow}，容易带来==**=={yellow}冗余和噪声==**=={yellow}，尤其在寿命后期会==**=={yellow}影响预测精度==**。

---

## 3. 为什么选择数据驱动方法

锂电池寿命预测主要可以分为=={yellow}模型驱动==和=={yellow}数据驱动==两类方法。

=={yellow}模型驱动的方法可解释性强但是在**复杂工况下落地难度高**==

模型驱动方法通常根据电池的电化学机理或等效电路=={green}建立数学模型，可解释性较强==，但需要准确的模型参数和复杂的参数辨识。在电池型号、温度和充放电工况发生变化时，模型的建立和校准成本也比较高。

数据驱动方法可以=={green}直接利用电池历史充放电数据==，学习多维监测特征与容量退化之间的非线性关系。这个项目具有=={pink}**NASA公开的全生命周期电池数据**==，因此采用数据驱动方法=={yellow}更适合开展**特征分析**和**时序预测**研究。==

这并不意味着数据驱动方法一定优于机理模型，而是基于当前的数据条件和研究目标，我们更关注如何从历史数据中提取有效退化信息，并验证预测方法的可行性。

## 4. 为什么还要部署到昇腾边缘端

锂电池寿命预计系统在=={green}实际工程中==，=={green}不可能把数据**传回 PC **或**云端**再分析==，而是需要**=={yellow}低功耗、高实时、可嵌入==****=={yellow}**的**就地推理能力**==。

如果模型只在 PC 上跑通，那它更多还是一个离线算法；但放到边缘板端以后，它才更接近真实应用场景，因为 BMS 需要的是就地推理、快速响应和可嵌入部署。

---

在论文研究阶段，算法主要运行在PC环境中，可以验证预测方法是否有效，但这仍然属于离线算法实验。

如果希望这套方法进一步服务于BMS，就需要考虑模型能否**=={yellow}在资源受限的边缘设备上运行==**。实际应用中，完全依赖云端会受到网络连接、数据上传和隐私等因素影响；将模型部署在靠近电池管理系统的边缘侧，可以减少对网络的依赖，使设备在离线或弱网条件下仍然具备本地分析能力。

锂电池寿命预测并不是毫秒级控制任务，因此边缘部署的核心价值不是追求极低时延，而是验证模型的本地可运行性、弱网可用性和嵌入式集成能力。

所以，在完成GRA、PCA和LSTM预测算法研究后，我进一步将PC端模型迁移到华为Atlas 200I DK A2平台，希望打通从算法研究到端侧部署的完整链路，**=={yellow}把一个PC端离线模型转化为能够在昇腾边缘设备上独立运行的推理系统==。**

**---------------------------------------------------------------------------------------------------------------------**

### =={green}为什么是华为昇腾这块板，为什么说它适配这个项目？==

第一，这个平台本身就是围绕**=={yellow}推理部署==**来设计的。

Ascend310B 的 CANN 架构会在 **ONNX 转 OM 时做=={yellow}算子编译优化==**，=={yellow}提前确定**内存分配、并行策略和算子融合**规则==。这说明这块板子的优势不只是“能运行代码”，而是它对推理执行有专门的编译和优化机制。

第二，它的**=={yellow}工具链和部署链条是完整的==**。
...
总之，选择昇腾这个平台，=={pink}不仅是因为它==**=={pink}针对推理部署做了优化==**=={pink}，而且还==**=={pink}提供了从模型转换、编译优化到板端推理的一整套工具链==**=={pink}，适合==**=={pink}把算法做成真正可部署的边缘推理应用==**=={pink}。==

### =={green}从“理论算法”到“可用项目”的角度==
...
> =={yellow}整个项目包含**算法研发侧**和**工程部署侧**两条主线==。=={green}**算法侧**要解决的是如何**从多维充放电数据**中**提取有效退化信息**，并利用LSTM完成**容量、EOL和RUL预测**；===={pink}**工程侧**要解决的是如何把**PC端训练好的动态图模型**转换成**昇腾平台能够执行的静态OM模型**，并在Atlas 200I DK A2上形成**完整的推理链路**==。我负责打通这两条主线，使项目从论文中的预测方法进一步发展为可以在边缘板端运行的推理系统。

## 1. 整体任务

先说明项目最终要做成什么：

> 构建一套面向BMS的锂电池剩余寿命预测方案，从历史充放电数据中提取退化特征，建立容量预测模型，并将模型部署到昇腾边缘平台，形成从=={yellow}**数据处理**、**模型训练**到**板端推理**和**结果验证**的完整链路。==

这里需要强调，任务不是单纯“训练一个LSTM”，而是包含两条连续主线：

- **算法主线**：提高电池容量和EOL预测效果；
- **工程主线**：把PC端算法迁移成昇腾板端能够运行的推理系统。
...
### 2.=={yellow}{green} 数据预处理==

论文实验中的主要预处理包括：

- **=={yellow}删除重复==**的充电记录；
- 对**=={yellow}缺失==****=={yellow}**或==**===={yellow}无效数据=={yellow}**进行检查，并使用对应**=={yellow}特征的均值=={yellow}**=={yellow}进行**==****=={yellow}补充==**；
- 按充放电循环整理电压、电流、温度和容量数据；
- 对提取后的特征和容量进行**=={yellow}Z-score标准化==**。
...
**=={green}标准化的目的==**不是改变特征之间的退化关系，而是=={yellow}消除==电压、电流、温度和时间等****=={yellow}***_**不同特征**因为_*量纲**造成的**数值尺度差异**==，=={yellow}避免**数值范围较大的特征**在PCA和模型训练中**占据过大权重**。==

在规范实现中，均值、标准差、GRA筛选结果和PCA变换矩阵都应该只根据训练集计算，然后原样应用到验证集、测试集和板端，避免引入未来数据泄漏。

### 3. 11维退化特征

项目=={pink}从充电和放电阶段提取了11维退化特征==：
...
## =={pink}A2：GRA特征筛选与PCA降维==

### =={green}1. 为什么不能把11维特征全部输入LSTM==

11个特征虽然都来自充放电过程，但它们=={yellow}对容量退化的**敏感程度不同**==，而且=={yellow}部分特征之间可能**表达相似信息**。==

如果直接=={green}全部输入模型，可能出现==：

- =={pink}**弱相关特征**引入**噪声**==；
- =={pink}**高相关特征**之间存在**重复信息**==；
- =={pink}**增加**===={pink}**模型参数**和**训练难度**；==
- =={pink}影响模型在寿命**中后期**对**关键**退化趋势的**判断**==。

因此，我=={yellow}采用**GRA**和**PCA**分两步优化输入。==

### ----------------------------------------------------------------------------------------

### =={pink}2. GRA：筛选与容量退化强相关的特征==

=={green}GRA，也就是**灰色关联分析**==，=={yellow}通过**比较特征序列**与**容量序列**随循环**变化的趋势相似程度**，**衡量**每个**特征与容量退化**的**关联性**。==
...
\xi_i(k)
$$
=={yellow}**关联度越大**，说明**特征变化趋势**与**容量退化趋势**越接近。==

项目以**=={pink}0.75作为筛选阈值，从11维特征中保留9维==**，剔除了：
...
### =={pink}3. PCA（主成分分析）：进一步压缩冗余信息==

经过=={green}GRA筛选后，**剩余9个特征**都与容量退化具有**较强关联**==，=={yellow}但它们之间仍然可能存在**信息重叠**==。例如平均放电电压、放电电压下降量和恒流放电时间，都=={yellow}可能从不同角度描述相近的退化现象==。
...
=={yellow}项目中**前四个主成分**的**累计方差贡献率**达到**98.65%**，因此将9维特征压缩成**4维健康因子**，作为**LSTM的输入**。==

---

### =={pink}4. 为什么GRA和PCA需要同时使用==

GRA和PCA解决的不是同一个问题：

- GRA=={green}利用容量标签==，回答“=={yellow}哪些特征与容量退化**更相关**==”；
- PCA=={green}不使用容量标签==，回答“如=={yellow}何**消除**入选特征之间的**冗余**和**共线性**==”。

因此，整体逻辑是：

> GRA先**去掉**与预测目标**关系较弱的特征**，PCA再**压缩**保留特征中的**重复信息**。
...
## =={pink}A3：LSTM时序建模与寿命预测==

### =={green}1. 为什么使用LSTM==

电池在当前循环的健康状态不仅与当前采集值有关，还与此前多个循环形成的长期退化趋势有关，因此这是一个=={yellow}**典型的时序预测问题**==。

=={green}普通RNN**能够处理序列**==，但在=={green}**较长序列反向传播**==过程中容易出现=={green}**梯度消失**或**梯度爆炸**==，=={green}难以稳定保留**较早循环的信息**。==

LSTM在**=={pink}RNN基础上==**=={pink}增加了==**=={pink}细胞状态==**=={pink}以及==**=={pink}遗忘门==************=={pink}****、==****==={pink}输入门**=={yellow}****=={pink}和==******=={pink}输出门==**=={pink}：==

- =={yellow}遗忘门决定**保留多少历史信息**；==
- =={yellow}输入门决定**写入多少当前信息**；==
- =={yellow}输出门决定**向下一时刻输出多少状态**==。

这种=={pink}门控结构能够**缓解长序列中的梯度问题**==，更适合学习电池长期、非线性的容量退化规律。这里应表述为“缓解”，不能说LSTM完全解决了梯度消失和梯度爆炸。
...
> **=={yellow}先完成容量回归==**=={yellow}，再==**=={yellow}根据容量==**=={yellow}预测曲线==**=={yellow}推导EOL==**=={yellow}和==****==**={yellow}RUL**=={yellow}****=={yellow}。==
...
- 优化健康因子模型使用**=={green}50个LSTM神经元==**；
- 最大**=={green}训练轮数为200==**；
- 使用**=={green}Adam优化器==**；
- **=={green}初始学习率为0.01==**；
- 使用**=={green}MSE作为训练损失==**=={green}；==
- 分别进行**=={green}21步和42步预测==**实验。
...
论文中比较了21步和42步两种预测策略。这里的=={yellow}“21步、42步”主要表示**预测跨度**==，即使=={yellow}用**当前及历史信息**预测**之后21/42**==**循环=={yellow}的容量==**。

=={green}预测跨度越大==，提前量越高，但=={green}误差也更容易累积==；预测=={green}跨度越小==，能够=={green}更细致地跟踪局部退化变化。==

需要注意，论文中的“=={pink}**预测跨度21步**”和**部署模型输入Shape**中的“**序列长度21**”是两个概念：==

- =={yellow}预测跨度表示**向未来预测**多少个循环；==
- =={yellow}输入序列长度表示**每次模型推理**使用多少个**历史循环**。==
...
**=={green}为了证明算法侧设计的GRA+PAC方法有效，做如下对比实验==**

#### =={pink}对照一：健康因子构造方式==

比较：

- =={yellow}**一维综合健康因子**＋LSTM；==
- =={yellow}**GRA＋PCA构造的多维优化健康因子**＋LSTM==。

目的在于=={green}验证**预测提升来自特征优化**==，而不只是LSTM本身。
...
该指标直接反映**=={green}预测寿命终点与真实寿命终点相差多少个循环==**，比单纯使用全周期容量误差更贴近寿命预测目标。
...
算法侧解决的是“如何预测”，=={yellow}工程部署侧解决的是“如何让训练完成的模型在昇腾边缘设备上运行”。==
...
如果**=={green}直接在ARM板端使用PyTorch加载==`.pth`=={green}，即使能够运行，也主要是在CPU上执行，不能使用Ascend NPU的推理能力==**，而且板端还要安装完整的PyTorch依赖。
...
=={yellow}ONNX是一种与**训练框架**和**推理框架**相对**解耦**的模型中间表示。==
...
### =={pink}4. ==`.pth`=={pink}导出ONNX的过程==

#### =={pink}第一步：重建与训练时一致的网络结构==
...
如果**=={yellow}网络结构==**、隐藏层大小或者层数**=={yellow}与训练时不一致==**，`load_state_dict()`**=={yellow}会出现参数Shape不匹配==**；即使通过部分加载，也可能导出一个语义错误的模型。
...
- =={green}第一个维度是Batch Size；==
- =={green}21表示历史序列长度；==
- =={green}4表示PCA处理后的健康因子维度；==
- =={green}100表示LSTM隐藏层维度。==

**=={yellow}Dummy Input的作用==**=={yellow}是让PyTorch在==**=={yellow}导出过程中执行一次前向传播==**=={yellow}，并==**=={yellow}据此建立ONNX静态计算图==**=={yellow}。==
...
=={yellow}opset表示ONNX==**=={yellow}算子规范版本==**=={yellow}，==**=={yellow}不同版本==**=={yellow}算子的==**=={yellow}输入shape==**=={yellow}和==**=={yellow}语义==**=={yellow}可能==**=={yellow}不同==**=={yellow}。==

不同opset下，LSTM、Gather、Squeeze等算子的属性和表达方式可能不同。昇腾ATC不一定支持最新版本中的所有算子，因此不能盲目选择最高版本。

=={green}项目选择opset 12，主要是在以下两方面之间取平衡：==

- =={yellow}能够**完整表达**当前**LSTM模型**；==
- =={yellow}与当时使用的**PyTorch**和**CANN工具链**具有较好的**兼容性**。==

当前**=={green}ONNX模型==**=={green}经过检查，主要包含==**=={green}LSTM==**=={green}、==**=={green}Transpose==**=={green}、==**=={green}Squeeze==**=={green}、==**=={green}Gather==**=={green}和==**=={green}Gemm==**=={green}等算子，==计算图结构相对简单。

### ----------------------------------------------------------------------------------------

### =={pink}6. 为什么不使用动态Shape而是静态shape==

部署版本没有设置`dynamic_axes`，而是固定为：
$$
\operatorname{shape}(x)=[1,21,4]
$$
=={green}固定Shape以后，ATC可以在**编译阶段提前确定计算图细节**==：
...
=={yellow}有利于===={yellow}**推理模型优化**与**减少推导开销**==
...
- =={yellow}使用不兼容的opset，可能产生ATC不支持的算子；==
- =={yellow}使用动态Shape，可能导致**LSTM无法编译**或者**性能不稳定**；==
- =={yellow}不显式暴露==`h0/c0`=={yellow}，**模型每次调用只能从固定初始状态开始**。==
...
我首先将PyTorch训练得到的**`.pth`模型转换为ONNX**。因为`.pth`主要保存模型权重，所以导出前需要**重建与训练时一致的网络结构**、**加载权重**并调用`eval()`**切换到推理模式**，因为导出 ONNX 是推理图，不是训练图。

考虑到昇腾NPU采用静态编译，我没有使用动态Shape，而是**将输入固定为`[1,21,4]`。同时把LSTM的`h0/c0`设置为显式输入，把`hn/cn`设置为输出**。

导出时**选择opset 12**，主要是为了兼顾PyTorch导出和CANN工具链的兼容性。

---

## =={pink}A5：ATC编译与OM模型生成==
...
**=={yellow}ATC，也就是Ascend Tensor Compiler，负责将通用ONNX图编译成与目标昇腾芯片匹配的OM模型。==**
...
=={yellow}通过==**=={yellow}SSH远程登录板端==**后，可以使用`npu-smi info`确认NPU型号和运行状态，并检查CANN环境变量、Python架构及依赖库是否与aarch64平台匹配。
...
| `--framework=5` | =={yellow}表示输入模型格式为ONNX== |
...
=={yellow}不同Ascend芯片==**=={yellow}支持的算子==**=={yellow}、==**=={yellow}AI Core架构==**=={yellow}和==**=={yellow}内存结构==**=={yellow}可能不同。==**=={yellow}atc会做编译优化，针对不同硬件做的优化也会不同，得到的OM也不同。==**
...
=={yellow}电池容量预测属于**连续值回归**==，=={yellow}**较小的数值误差**也可能**影响EOL**越阈**位置**==。
...
=={yellow}OM是昇腾平台的离线模型格式。==

它不只是ONNX文件的简单改名，而是经过ATC编译后形成的=={yellow}硬件相关==执行模型，其中包含：
...
### =={pink}2. 为什么LSTM比普通前馈模型更难部署==

=={yellow}**普通前馈网络**通常是**一次输入对应一次输出**，**每次推理**之间**相互独立**==。

=={green}**LSTM**除了**当前输入**(x_t)，还**依赖上一时刻**的：==

- **=={green}隐藏状态(h_{t-1})；==**
- **=={green}细胞状态(c_{t-1})。==**

=={pink}在PyTorch中==，=={pink}这些状态==可以在Python运行逻辑中=={pink}**动态创建和传递**；===={yellow}但OM是**静态计算图**，每次**模型调用**都是一次**固定的输入到输出映射**==，不会自动保存Python对象中的状态。

如果=={pink}不做处理，**每次调用**模型都会**从零状态开始**==，**=={pink}跨次推理==**=={pink}的==**=={pink}时序上下文==**=={pink}可能==**=={pink}丢失==**=={pink}。==

### ----------------------------------------------------------------------------------------

### =={pink}3. 将状态显式设置为模型输入接口==

为了解决这个问题，我将**=={yellow}LSTM模型改为三个显式输入==**：
...
这样，=={yellow}上一轮输出的==`hn/cn`=={yellow}可以作为下一轮的==`h0/c0`=={yellow}，由**运行时显式维护状态。**==

### ----------------------------------------------------------------------------------------

### =={pink}4. 固定滑动窗口的构造==

=={yellow}板端使用==**=={yellow}长度为21的队列==**=={yellow}保存==**=={yellow}最近的健康因子==**=={yellow}：==
...
=={yellow}每次**加入新的健康因子**，同时**移除最早的数据**，再将**窗口整理**为：==

=={yellow}[ [1,21,4] ]==

这种方式=={green}**满足**了OM模型的**固定Shape要求**==，=={green}也能够**用滑动窗口近似模拟动态记忆**==。

### ----------------------------------------------------------------------------------------

### 5. 为什么不直接使用=={pink}单个时间步==

如果每次只输入当前一个循环的4维健康因子，那么=={pink}模型**只能依赖**==`h/c`=={pink}**状态保存历史信息**。==
...
=={green}LSTM部署的==**=={green}特殊点==**=={green}是它==**=={green}不===={green}仅依赖当前输入==**=={green}，==**=={green}还依赖上一时刻的隐藏状态和细胞状态==**=={green}。===={pink}但OM模型每次调用都是独立的静态图，**不会自动保存**Python**运行时状态**==。

因此，=={yellow}我把==`h0/c0`=={yellow}**显式设置为模型输入**，把==`hn/cn`=={yellow}设置为输出。**板端每次推理完成后，将新的状态保存并传入下一次推理**。同时使用**21步固定滑动窗口近似模拟动态记忆**，使输入Shape始终保持==`[1,21,4]`=={yellow}。==
...
=={green}因此，==**=={green}板端部署的交付物==**=={green}不仅是一个==**=={green}OM文件==**=={green}，还包括==**=={green}围绕OM模型建立的推理程序==**=={green}。==
...
=={yellow}ACL Lite是对ACL常用流程的**Python封装**==。项目使用它简化资源初始化、模型加载和执行流程，但**=={yellow}底层实际推理仍由ACL Runtime完成==**=={yellow}。==
