...
# **公司**：上海禾赛科技有限公司**部门**：软件部——=={yellow}**雷达功能**与**功能安全**组==**岗位**：嵌入式软件工程师**产品**：新一代车载激光雷达**硬件平台**：=={yellow}**芯驰E3116**/9==系列=={yellow}**双核Cortex-R5** MCU==**软件平台**：=={yellow}**AUTOSAR Classic**== + =={yellow}**ETAS RTA-OS**==**项目背景**：参与小米、理想、吉利等客户的量产项目**主要方向**：=={yellow}**MCU功能开发**==、=={yellow}**功能安全诊断**==、=={yellow}**调试工具**==和软件版本交付**涉及到的产品**：ETX（**=={yellow}前向主远距激光雷达==**，800线）、FTX （**=={yellow}纯固态超广角补盲雷达==）**文档中同时涉及E3116和E3119等具体型号。面试中可以统一表述为：芯驰E311X系列双核Cortex-R5平台，不同项目使用的具体芯片型号和软件配置略有差异。

## =={pink}2. 车载激光雷达是做什么的==

# 车载激光雷达是=={yellow}自动驾驶和ADAS系统中的环境感知传感器。==它通过=={yellow}**发射激光**并**接收**目标物体**反射**回来的**光**，测量激光往返时间，计算物体与车辆之间的距离，**最终形成三维点云**。==基本链路是：
...
域控制器再**基于点云**完成**目标检测、障碍物识别和多传感器融合**。
...
例如，=={pink}雷达上电后，MCU需要：===={green}初始化**芯片**、**RTA-OS**和基础软件**BSW**==；-------------整机初始化------------------=={green}读取**Flash**中的**程序**及标定**参数**==；=={green}加载和配置**FPGA**==；=={green}初始化**SPAD芯片**、发射高压**TX**、接收高压**RX** 和**扫描器**==；=={green}按照**状态机**逐步进入正常**工作模式**；==-------------工作状态------------------=={green}**持续监控**电压、温度、通信和存储数据==；=={green}出现**异常时记录DTC**，必要时**限制功能**或**关闭激光**；==-------------诊断与调试------------------=={green}**响应上位机PTC**和**整车的UDS诊断请求**==；=={green}支持**产线调试**、**软件升级**==及版本交付。因此，MCU软件的定位可以概括为：
