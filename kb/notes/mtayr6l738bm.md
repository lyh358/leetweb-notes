...
**GPS 字节流入 UART 硬件** FIFO，**DMA** 硬件**自动**把字节**搬运到内存缓冲区**，不需要 CPU 参与搬运。DMA **完成 / 半满 / 超时**，才**产生**一次**中断通知 CPU**。CPU **一次拿到一大段** GPS 数据。
...
### =={pink}5.5 为什么GPS单独使用任务==

GPS与DHT22、HX711不同：

- =={yellow}DHT22和HX711按**秒级周期主动轮询读取**；==
- =={yellow}GPS会**持续输出字节流**，需要**毫秒级检查UART缓冲器（如果使用中断方式读取）并持续接收**。==
- =={yellow}二者都是==**=={yellow}时序敏感==**=={yellow}的任务，==**=={yellow}解耦==**=={yellow}防止==**=={yellow}相互阻塞==**
...
| 组件 | 作用 |
| --- | --- |
| **=={yellow}esp_camera==** | =={yellow}ESP32 **官方摄像头驱动库**==，封装了初始化、配置、采集流程 |
| **OV2640** | 200万像素 CMOS 摄像头传感器，输出 DVP 并行视频数据 |
| **=={yellow}I²S==** | ESP32 的 =={yellow}I²S 外设，这里被==**=={yellow}复用为并行摄像头接口==**（接收 DVP 数据） |
| **DMA** | 直接内存访问，=={yellow}数据从 I²S 自动搬运到内存==，**CPU 不参与逐字节拷贝** |
| **=={yellow}帧缓冲区==** | =={yellow}内存中的一块区域，存一帧图像==（如 320×240 RGB565 = 150KB） |
