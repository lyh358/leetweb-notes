...
将=={yellow}图像采集单独放在ESP32-CAM上==，=={yellow}主要是因为图像的数据量、缓存占用和网络传输开销远高于普通传感器数据==。如果摄像头和全部传感器都=={yellow}集中在同一主控==上，图像采集和发送容易=={yellow}占用较多CPU、内存和网络资源，影响普通遥测的稳定性==。=={yellow}拆成两个节点==后，传感器遥测和图像链路可以=={yellow}分别调试，故障边界也更加清晰==。
