...
> 存储层级（由外→内）：GM 全局内存 (LPDDR4) → L2 (片上二级缓存，AI‑Core 集群共享) → AI‑Core 私有：L1 Buffer → L0A/L0B/L0C、UB (Unified Buffer)
> 
> MTE 搬运引擎：
> 
> 
...
> 2. **Vector 只能直接访问 UB**，向量运算必须把数据放进 UB。Vector 不能直接读写 L1/L0 系列 Buffer.。
