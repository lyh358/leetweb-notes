...
在=={green}**不同的充放电循环**==中，电压、电流、温度、充放电时间等=={green}数据都会发生变化==，而且=={green}不同特征==对电池=={green}老化的**敏感程度**==并不相同。部分特征与容量衰减关系=={green}较强==，部分特征可能包含=={green}冗余或噪声==。如果直接把所有监测数据送入预测模型，不但会增加模型复杂度，还可能干扰模型对真实退化趋势的学习。

另外，电池容量曲线可能出现=={green}**局部波动**和**短期容量回升**==现象。特别是在=={green}电池寿命的中后期==，也就是接近EOL的阶段，容量变化对寿命终点判断更加敏感。即使预测值只有较小偏差，也可能导致预测的EOL循环位置发生明显变化。

因此，这个项目需要解决的不只是“能不能拟合容量曲线”，更重要的是：

- 如何从=={pink}**多维**充放电数据==中找到=={pink}**真正**能够**反映电池退化**的特征==；
- 如何减少=={pink}**特征之间**==的=={pink}**冗余**和**噪声**==；
- 如何学习电池=={pink}**长期、非线性**的**退化规律**==；
- 如何提高=={pink}寿命中后期==，尤其是=={pink}**EOL附近**的**预测可靠性**==。
