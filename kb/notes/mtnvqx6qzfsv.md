...
互斥锁属于 RTOS 内核对象，专门**=={yellow}用于保护多任务之间独占访问共享资源==**；
...
> 1. =={yellow}互斥锁有任务所有权==，=={green}只有持有者可以释放==；=={yellow}二值信号量没有所有者==，=={green}任意任务都可以 give。==
> 2. =={yellow}互斥锁自带优先级继承机制==，专门用来解决优先级反转，=={yellow}用于资源互斥保护==；=={yellow}二值信号量没有优先级继承，多用于事件通知。==
