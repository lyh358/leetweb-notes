...
1. **=={yellow}Stub 层（桩代码）==**：=={green}根据== `.proto` =={green}**文件生成**，客户端**直接调用本地函数**，屏蔽网络细节==
2. **=={yellow}Protobuf 序列化层==**：**=={green}对象 ↔ 二进制字节流==**，体积小、编解码快
...
2. **=={yellow}TCP多路复用==**：=={green}**一条 TCP 连接**上，可以**并发跑多个gRPC请求流 (stream)**，**不用频繁建连**；请求**互不阻塞**==
3. **=={yellow}四种调用模式==**：=={green}支持（**一元**、**服务端流**、**客户端流**、**双向流**）==，HTTP1.1 做不到双向流
...
- =={yellow}数据转紧凑二进制==，对比 JSON：体积更小、序列化反序列化速度更快；缺点是可读性差。

### =={pink}3. 四种调用模式==

1. **=={yellow}一元 RPC（Unary）==**：=={green}客户端发 **1 请求** → 服务返回 **1 响应**==（最常用，类似普通 http 接口）
2. **=={yellow}服务端流式 RPC==**：=={green}**客户端**发 **1 请求**，**服务端**返回**一串数据流**==（例如大量数据推送）
3. **=={yellow}客户端流式 RPC==**：=={green}**客户端**连续**发一堆请求**，服务端**返回 1 个应答**==
4. **=={yellow}双向流式 RPC==**：=={green}两边**同时互发数据流**（聊天、实时数据）==
