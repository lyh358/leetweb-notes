...
=={yellow}在===={yellow}**不同的充放电循环**===={yellow}中，电压、电流、温度、充放电时间等===={yellow}数据都会发生变化===={yellow}，而且===={yellow}不同特征===={yellow}对电池===={yellow}老化的**敏感程度**===={yellow}并不相同。部分特征与容量衰减关系===={yellow}较强===={yellow}，部分特征可能包含===={yellow}冗余或噪声===={yellow}。如果直接把所有监测数据送入预测模型，不但会增加模型复杂度，还可能干扰模型对真实退化趋势的学习。==

=={yellow}另外，电池容量曲线可能出现===={yellow}**局部波动**和**短期容量回升**===={yellow}现象。特别是在===={yellow}电池寿命的中后期===={yellow}，也就是接近EOL的阶段，容量变化对寿命终点判断更加敏感。即使预测值只有较小偏差，也可能导致预测的EOL循环位置发生明显变化。==

=={yellow}因此，这个项目需要解决的不只是“能不能拟合容量曲线”，更重要的是：==

- =={yellow}如何从===={yellow}**多维**充放电数据===={yellow}中找到===={yellow}**真正**能够**反映电池退化**的特征===={yellow}；==
- =={yellow}如何减少===={yellow}**特征之间**===={yellow}的===={yellow}**冗余**和**噪声**===={yellow}；==
- =={yellow}如何学习电池===={yellow}**长期、非线性**的**退化规律**===={yellow}；==
- =={yellow}如何提高===={yellow}寿命中后期===={yellow}，尤其是===={yellow}**EOL附近**的**预测可靠性**===={yellow}。==
