...
1. =={yellow}外设产生中断请求==后，先=={pink}送到 ==**=={pink}NVIC==**，NVIC 判断该中断=={pink}是否使能==、=={pink}优先级是否满足===={green}响应条件。==
...
1. STM32 用**=={yellow}定时器==**=={yellow}产生固定频率的 PWM 波，核心参数是 ==`PSC`=={yellow}、==`ARR`=={yellow}、==`CCR`。
...
1. Cortex‑M =={yellow}把一段 ==**=={yellow}位带区==**=={yellow} 的每一位，映射到 ==**=={yellow}位带别名区==**=={yellow}{yellow} 的一个 32 位地址==。
...
面试里一般既会问 **=={yellow}分辨率==**=={yellow}，也会追问 ==**=={yellow}实际精度是否真的能达到标称值==**=={yellow}。==

- **底层实现原理**

1. STM32 ADC=={yellow} 常见分辨率有 ==**=={yellow}12 位==**=={yellow}，部分型号还支持 ==**=={yellow}10/8/6 位==** 可配置，少数高端型号可到更高精度。
2. 以=={yellow} ==**=={yellow}12 位 ADC==**=={yellow}{yellow} ==为例，表示=={yellow}可把输入电压范围分成 (2^{12}=4096) 个等级==，数字范围通常是 **0~4095**。
3. 若=={yellow}参考电压是 ==`3.3V`=={yellow}，则 1 LSB 约等于 ==`3.3 / 4096 ≈ 0.8mV`=={yellow}，这叫==**=={yellow}分辨率==**=={yellow}。==
...
=={yellow}时钟树用来给 ==**=={yellow}CPU、总线、外设==**=={yellow}{yellow} 分配不同频率的时钟。==

面试问它，本质是看你是否清楚=={green} ==**=={green}系统时钟从哪来、怎么分给各模块==。**

- **底层实现原理**

1. =={pink}STM32 的时钟源一般有 ==**=={pink}HSI、HSE、LSI、LSE==**，=={green}高速时钟给系统和外设==，=={green}低速时钟常给 RTC、看门狗==。
2. =={yellow}系统时钟 ==`SYSCLK`=={yellow} 可以选择直接来自 HSI/HSE，也可以先经过 ==**=={yellow}PLL 倍频==**=={yellow}{yellow} 后再作为主时钟==。
3. `SYSCLK`=={yellow} 再往下分成 ==**=={yellow}AHB、APB1、APB2==**=={yellow}{yellow} 等总线时钟==，分别供内核、内存、不同外设使用。
...
3. STM32=={pink} 复位后虽然是从 ==`0x00000000`=={yellow}{pink} 开始取向量表，但芯片会把启动区域==**=={pink}重映射==**=={pink}到这个地址==。
...
- **应用场景**
=={yellow}STM32 串口接收常见有 ==**=={yellow}轮询、中断、DMA==**=={yellow}{yellow} 三种方式。==
...
2. 配置入口本质都在=={pink} ==**=={pink}RCC 时钟控制模块==**，核心寄存器一般是 `RCC->CFGR`。
...
1. STM32 的=={pink} Flash 是按 ==**=={pink}页（Page）或扇区（Sector）==**=={yellow}{pink} 管理==的，不是按字节独立擦除。
...
1. ADC 完成一次转换后，结果会先存到=={pink} ===={pink}**ADC 数据寄存器 DR（Data Register，数据寄存器）**==。
2. =={green}如果是 ==**=={green}轮询==**=={green} 或 ==**=={green}中断==**=={green} 方式，CPU 再去读取 DR，读出来后一般存到变量、数组或缓冲区里==。
3. =={green}如果是 ==**=={green}DMA==**=={green} 方式，DMA 会自动把 DR 里的数据搬运到 RAM 中指定的数组或缓存区==。
