...
### 使用CANN提供的msprof采集模型执行过程，重点关注：算子执行顺序；各Kernel的执行时间；Kernel调用是否过于细碎；Conv、MatMul等Cube算子；Softmax、GELU和逐元素运算等Vector算子；TransData格式转换；AI Core执行情况；数据搬运和计算之间是否存在等待。msprof用于回答两个问题：时间主要花在计算、数据搬运还是调度上？哪些算子或算子链值得优先优化？
