...
=={pink}编译时需要明确指定==**=={yellow}三组输入Shape==**=={yellow}、==**=={yellow}ND格式==**=={yellow}、==**=={yellow}目标芯片Ascend 310B4==**=={yellow}和==**=={yellow}精度模式==**。初版采用FP32，是为了先建立稳定的容量回归精度基线。OM生成后，我还会在**板端通过ACL Lite编写推理程序**实际**加载和执行**，而不是只以ATC编译成功作为部署完成的标准。
