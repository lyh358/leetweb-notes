...
**=={yellow}嵌入式终端==**=={yellow}（esp32s3+esp32cam）==**=={yellow}+  阿里云物联网平台  +  后端==**=={yellow}（mySQL+Flask）==**=={yellow}+  PC图像识别==**=={yellow}（yolov8）==**=={yellow}+  前端数据大屏  +  前端微信小程序==**

---

### 1. 嵌入式终端：采集蜂箱现场信息

嵌入式终端是整个系统的=={yellow}数据入口==，主要由 **=={yellow}ESP32-S3主控端==**=={yellow}和**ESP32-CAM图传端**===={yellow}组成。==
...
- =={yellow}**OLED**显示本地传感器数据和设备状态；==
- =={yellow}通过**Wi-Fi连接网络**；==
- =={yellow}通过**MQTT将遥测数据上传**至阿里云物联网平台；==
- =={yellow}**接收云端**下发的配置或维护**指令**。==

这些数据分别服务于温湿度预警、产蜜进度判断、收获提醒、蜂箱定位和防盗。

#### =={pink}ESP32-CAM图传端==

负责**蜂箱出入口的图像采集**与**上传PC端**进行识别：

- 拍摄蜂箱出入口的蜜蜂活动；
- =={yellow}提供**JPEG单帧图像或MJPEG视频流**；==
- =={yellow}将图像提供给PC端视觉程序处理==；
- =={yellow}与普通遥测链路分开，**避免图像数据占用主控的数据采集和通信资源**。==
...
### =={pink}2. 阿里云IoT平台：完成设备接入与数据中转==

阿里云IoT平台=={yellow}位于嵌入式终端和业务后端之间==，主要承担设备通信层的任务：

- =={yellow}管理==蜂箱终端的=={yellow}**设备身份**==；
- 完成=={yellow}设备**鉴权和连接**==管理；
- =={yellow}**接收**ESP32-S3通过**MQTT**上传的**遥测数据**==；
- =={yellow}按==设备或=={yellow}**主题**=={yellow}对数据进行=={yellow}**中转**和分发==；
- 将终端=={yellow}数据**转交**==给=={yellow}**后端**==服务；
- 支持云端向设备=={yellow}**下发**参数或维护**指令**==。
...
### =={pink}3. Flask后端与MySQL：处理数据和业务逻辑==

后端服务负责将设备数据转化成系统能够使用的业务数据。

#### =={yellow}数据接收与处理==

- =={yellow}接收阿里云IoT平台==转发的设备遥测；
- 解析温度、湿度、重量、GPS等数据；
- 将设备数据与具体蜂场、蜂农和蜂箱绑定；
...
Flask后端向=={yellow}小程序和数据大屏提供API==，例如：
...
### =={yellow}4. PC端YOLOv8：分析蜜蜂活动和胡蜂入侵==

图像数据量较大，而且目标检测模型的计算量超过普通ESP32终端的承载能力，因此视觉识别放在PC端运行。

YOLOv8模块主要完成：

- =={yellow}从ESP32-CAM获取蜂箱出入口图像；==
- =={yellow}检测画面中的蜜蜂和胡蜂==；
- 统计画面中的蜜蜂数量；
- 将统计结果转化为=={yellow}蜂群活跃度指标==；
- =={yellow}检测是否出现胡蜂；==
- 将活跃度和胡蜂预警=={yellow}结果交给后端==保存和展示。
...
### =={pink}5. 微信小程序：服务蜂农和云领养用户==

小程序面向两类用户：**=={yellow}蜂农==**=={yellow}和===={yellow}**普通消费者/认养用户**=={yellow}=={yellow}。==

#### =={pink}蜂农端功能==

- 查看自己=={yellow}管理的蜂箱==；
- =={yellow}**查看单个蜂箱**的温湿度、重量、活跃度和图像；==
- 查看蜂箱=={yellow}**异常预警**；==
- 查看蜂箱=={yellow}**位置**==；
- 通过地图=={yellow}**导航**==到指定蜂箱；
- 查看蜂箱=={yellow}领养和**订单情况**==。
...
我在团队中担任=={yellow}嵌入式终端负责人==**，负责智慧蜂箱设备端的**=={yellow}软硬件实现==**。**=={yellow}硬件方面==**，我完成了ESP32-S3、ESP32-CAM及温湿度、称重、GPS、OLED等=={yellow}模块的==**=={yellow}选型、接线和实物组装==**；**=={yellow}软件方面==**=={yellow}，我独立编写ESP32-S3主控端和ESP32-CAM图传端的程序==，并**=={yellow}**负责阿里云IoT平台的**设备**注册**、**物模型定义**和**通信配置**。==
