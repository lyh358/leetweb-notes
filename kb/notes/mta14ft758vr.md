...
=={yellow}在**编译期**（模板实例化阶段），对于某个**消息类型T**，T在尝试使用**实际类型**代替偏**特化模板的形参**的过程中（此时没有进入结构体内部代码），如果**产生了非法类型**/表达式，此时编译器在编译期（模板实例化阶段，对于某个消息类型T，T在尝试使用实际类型代替偏特化模板的形参的过程中（此时没有进入结构体内部代码），如果产生了非法类型/表达式，此时编译器不报编译错误，而是把这个模板特化版本从重载候选集里丢弃，编译器继续去找其它可行版本。误，而是把这个模板特化版本从重载候选集里丢弃，编译器继续去找其它可行版本。==
