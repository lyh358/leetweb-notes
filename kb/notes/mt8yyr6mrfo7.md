...
**=={yellow}Dummy Input的作用==**=={yellow}是让PyTorch在==**=={yellow}导出过程中执行一次前向传播==**=={yellow}，并==**=={yellow}据此建立ONNX计算图==**=={yellow}。==
...
=={pink}---------------------------------------------------------------------------------------5. 为什么使用opset 12=={yellow}opset表示ONNX==**=={yellow}算子规范版本==**=={yellow}，==**=={yellow}不同版本==**=={yellow}算子的==**=={yellow}输入shape==**=={yellow}和==**=={yellow}语义==**=={yellow}可能==**=={yellow}不同==**=={yellow}。==不同opset下，LSTM、Gather、Squeeze等算子的属性和表达方式可能不同。昇腾ATC不一定支持最新版本中的所有算子，因此不能盲目选择最高版本。==
