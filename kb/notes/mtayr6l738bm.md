...
| 列组件作用**esp_camera**ESP32 官方摄像头驱动库，封装了初始化、配置、采集流程**OV2640**200万像素 CMOS 摄像头传感器，输出 DVP 并行视频数据**I²S**ESP32 的 I²S 外设，这里被**复用为并行摄像头接口**（接收 DVP 数据）**DMA**直接内存访问，数据从 I²S 自动搬运到内存，**CPU 不参与逐字节拷贝****帧缓冲区**内存中的一块区域，存一帧图像（如 320×240 RGB565 = 150KB） |  |  |
| --- | --- | --- |
| 组件 | 作用 |  |
