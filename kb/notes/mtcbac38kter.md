...
=={yellow}ROS==**=={yellow}不是==**=={yellow}类似==**=={yellow}Linux==**=={yellow}或===={yellow}**Windows**===={yellow}的===={yellow}**传统操作系统**=={yellow}=={yellow}，而是==**=={yellow}运行在Linux之上==**=={yellow}的====={yellow}**={yellow}机器人软件框架**=={yellow}=={yellow}。==
