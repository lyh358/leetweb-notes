...
**=={green}GPS 字节流入 UART 硬件==** =={green}FIFO，DMA 硬件自动把字节搬运到内存缓冲区，==**=={green}不需要 CPU 参与搬运==**=={green}。==DMA **完成 / 半满 / 超时**，才**产生**一次**中断通知 CPU**。=={yellow}CPU **一次拿到一大段** GPS 数据。===={green}ESP32 UART 支持 DMA，不需要 CPU 搬运字节。==
