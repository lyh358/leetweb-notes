...
哈佛：指令和数据分开存储、独立总线，可同时取指和取数。冯诺依曼：指令和数据共用存储和总线，结构简单但访存冲突。- STM32 比 51 更适合跑 RTOS 的原因

STM32 有 Cortex-M 内核的 **NVIC 中断嵌套** + **SysTick** +** PSP/MSP 双栈指针** + **硬件除法**，上下文切换快。
---
...
32 位单片机一般通过 =={pink}**PWM 改变电机两端的平均电压**来实现调速==。
...
1. =={pink}STM32 的时钟源一般有 ==**=={pink}HSI、HSE、LSI、LSE==**，=={green}高速时钟给系统和外设==，低速时钟常给 RTC、看门狗。
