...
1. **输入迭代器**：只读，只能单向 ++，如`istream_iterator`
2. **输出迭代器**：只写，只能单向 ++，如`ostream_iterator`
3. **前向迭代器**：可读写，只能单向`++`，单向链表`forward_list`
4. **双向迭代器**：支持`++`、`--`，**list 的迭代器就是双向迭代器**，可以前进后退，但不能直接加减数字偏移。
5. **随机访问迭代器**：`++ -- + - []`都支持，可以像指针一样偏移，**vector 迭代器属于随机访问迭代器**。
