...
=={green}初始化**芯片**、**RTA-OS**和基础软件**BSW**==；-------------整机初始化------------------=={green}读取**Flash**中的**程序**及标定**参数**==；

=={green}加载和配置**FPGA**==；

=={green}初始化**SPAD芯片**、发射高压**TX**、接收高压**RX** 和**扫描器**==；

=={green}按照**状态机**逐步进入正常**工作模式**；==-------------工作状态------------------

=={green}**持续监控**电压、温度、通信和存储数据==；

=={green}出现**异常时记录DTC**，必要时**限制功能**或**关闭激光**；==-------------诊断与调试------------------

=={green}**响应上位机PTC**和**整车的UDS诊断请求**==；

=={green}支持**产线调试**、**软件升级**==及版本交付。因此，MCU软件的定位可以概括为：
