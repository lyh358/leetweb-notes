...
车载激光雷达是=={yellow}自动驾驶和ADAS系统中的**环境感知传感器**。==
...
- =={yellow}**FPGA**==擅长=={yellow}**高速**==、并行和=={yellow}**时序**==严格的工作，例如=={green}**激光时序**==、=={green}**回波采样**==以及部分=={green}**点云数据**处理==；
- =={yellow}**MCU**==负责=={yellow}**整机控制**==，**包括**=={green}**模式与电源控制**==**、=={green}**外设驱动**==、=={green}Flash与===={green}参数管理==**、=={green}**通信诊断**==和=={green}**功能安全，PTC产线遥测**==。
...
- =={green}**RTA-OS任务调度**==；
- =={green}**DCM（Diagnostic Communication Manager，诊断通信管理器）**==；
- =={green}**DEM（Diagnostic Event Manager，诊断事件管理器）**==；
- =={green}**Flash**==和NvM存储管理；
- =={green}**以太网**==和DoIP协议栈；
- EcuM、BswM等系统管理。
...
- =={green}**QM**和**Trusted分区**之间通过**IOC（Inter‑OS‑Application Communication）跨分区机制通信**；==
...
> 核心概念：**=={green}ASIL 等级（A/B/C/D）==**- ASIL‑D：最高安全等级（刹车、转向、动力）
> 
> 
> - ASIL‑A：最低等级
...
> =={green}车载可靠性 C 语言编码规则集，约束 C 语言危险特性。==
...
这一部分主要位于MCU软件的=={yellow}ASW和CDD层==，并=={yellow}通过RTE与BSW、MCAL及硬件交互==。

### 主要工作

- 在AUTOSAR分层架构下进行=={yellow}嵌入式C==功能开发；
- 根据RTE接口接入周期任务和模块间通信；
...
## =={pink}4. 工作线三：产线调测工具的协助开发==

### =={pink}PC1300遥测UDP协议改造==

原有=={yellow}PC1300遥测数据在**多台雷达同时接入时，缺少足够明确的设备唯一标识**。==
...
=={green}使**上位机能够区分不同雷达或芯片的数据来源**，支持多设备并发监控和问题追溯。==

### =={pink}PTC调试指令==

针对芯片电源域和SPAD电压通道调试需求，我=={yellow}**开发了上位机PTC指令**==，支持：
...
- ADS/Tasking；
- CMake；
- 串口、日志和=={yellow}**wireshark抓包（最终产物检查：点云包）**==工具；
- MISRA C编码规范；
- AUTOSAR MemMap内存段管理。
