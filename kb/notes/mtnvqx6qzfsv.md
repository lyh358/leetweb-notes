...
互斥锁属于 RTOS 内核对象，专门**=={yellow}用于保护多任务之间保护共享资源==**；
...
防止中间优先级的任务抢占 CPU，以此缓解优先级反转；=={green}锁释放之后，持有锁的任务恢复原来的优先级。==
...
✅口述：互斥锁用于保护共享资源、二值信号量用于shi'jian't'z
...
1. =={yellow}互斥锁 = 带==**=={yellow}所有权==** =={yellow}+== **=={yellow}优先级继承==** =={yellow}的内核对象==；
2. =={yellow}拿===={yellow}不到锁，任务**挂入阻塞链表**；内部操作靠**临界区保证原子**==；
