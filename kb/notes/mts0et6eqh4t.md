...
- 增加**=={yellow}计算图复杂度==**，基线模型被编译=={yellow}成==**=={yellow}10个以上设备子图(CPU fallback+GM搬运+kernel调度==****=={yellow})==**，基线模型得到的静态计算图虽然**=={yellow}能运行==**=={yellow}，但是并==**=={yellow}不干净==**；子图之间需要**额外调度**和**数据传递**。；
- =={green}产生**冗余调度**==；
