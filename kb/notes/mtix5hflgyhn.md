...
### =={pink}5.1 流水线并行是怎么实现的==

=={yellow}核心方法是== **=={yellow}Tiling + Ping-Pong双缓冲==**=={yellow}。==

=={yellow}先把一个 Head 内的**大块数据沿 K 维切成多个 Chunk**==。例如原始 K 维为 64，可以切为两个 32：
...
这样，=={yellow}**Cube 计算当前块时**，**MTE 可以提前搬运下一块**，尽量把**数据搬运时间隐藏在计算时间**中==。

在当前 `64×64` 的=={green}小 Shape 下，只有两个 Chunk，因此流**水重叠空间有限。双缓冲属于辅助优化**==，=={yellow}主要收益仍来自== **=={yellow}Kernel融合和中间结果片上驻留==**。

### =={pink}5.2 为什么必须同步==

不同流水线可以异步执行。如果**没有同步，可能发生**：

- =={green}Cube 还没读取完 Ping，MTE 就将下一批数据覆盖进去===={green}；==
- =={green}数据还没搬入 L0A/L0B，Cube 就开始矩阵乘；==
- =={green}Score 还没计算完整，Vector 就开始 Softmax===={green}；==
- =={green}Cube 还没产生最终结果，MTE3 就写回 GM。==

因此，同步的本质是：

> **=={yellow}生产者通知消费者“数据已经准备好”，消费者处理完成后再通知生产者“缓冲区可以复用”。==**
...
#### =={pink}5.1.1 什么是Tiling？==

**=={yellow}Tiling（分块）==**=={yellow}就是==**=={yellow}把一个较大的计算任务切成若干个较小的数据块，再逐块完成计算。==**
...
这样做不是改变计算结果，而是=={green}**改变计算的执行方式，让每次只处理一小块数据**==。
...
=={yellow}**数学结果没有改变**，只是**将一次长度为 64 的矩阵乘累加**拆成了**两次长度为 32 的乘累加**。==

---

#### =={pink}5.1.5 为什么选择 32，而不是其他数字？==
...
=={green}需要注意：==**=={green}32 并不是普遍最优值==**。它是当前 `64×64` 原型 Shape 下结合硬件计算粒度、缓存使用和流水设计选择的 Tiling 参数。=={green}**实际工程中**通常由 Host 侧 **Tiling 根据 Shape 和片上存储容量**计算或**选择**。==
...
### =={pink}5.2 Ping-Pong双缓冲==

#### =={pink}5.2.1 什么是 Buffer==

数据从 GM（片外全局内存）搬进 AI Core 后，不能直接凭空交给 Cube 计算，需要先存放在片上存储空间中。

这块临时保存待计算数据的空间就是 **=={yellow}Buffer（缓冲区）==**。
...
=={yellow}这是流水线的启动阶段，也叫==**=={yellow}流水填充==**。因为一开始还没有数据，=={yellow}Cube必须等待第一次搬运==。

##### =={pink}第二阶段：计算与搬运重叠==

=={yellow}**Chunk 0准备好**后，**Cube计算Chunk 0**，**MTE搬运Chunk 1**：==
...
=={yellow}**Chunk 0计算完成**、**Chunk 1搬运完成**后，**MTE空闲**，**Cube计算Chunk 1**：==
...
=={yellow}这是==**=={yellow}流水排空阶段==**。

---

#### =={pink}5. 为什么叫 Ping-Pong==

=={yellow}“Ping-Pong”强调的是两套 Buffer== **=={yellow}交替使用==**=={yellow}，并不是 Ping 永远属于 Chunk 0、Pong 永远属于 Chunk 1。==
...
=={yellow}**两套 Buffer就像乒乓球一样来回切换**，所以称为 Ping-Pong 双缓冲。==

---

#### =={pink}5.2.6 它与 Tiling 的关系==
...
> **=={yellow}Tiling负责把任务切开，双缓冲负责让切出来的数据块流水执行==。**

如果没有 Tiling，就没有多个 Chunk 可供轮换；如果只有 Tiling、没有双缓冲，多个 Chunk仍可能只能串行搬运和计算。

---

#### =={pink}5.2.7 双缓冲并不能消除所有时间==

即使使用双缓冲，=={green}仍然存在==：

- 第一个 Chunk搬运时的**=={yellow}流水填充时间==**；
- 最后一个 Chunk计算时的**=={yellow}流水排空时间==**；
- 数据依赖造成的等待；
- =={yellow}**搬运时间**和**计算时间不完全相等**造成的**空隙**==。
...
算子中的=={yellow}流水线===={yellow}并行主要发生在**MTE和CUBE之间**==
...
#### =={pink}5.3.4 Chunk 0 和 Chunk 1 的计算结果如何合并==
...
=={yellow}二者重叠后，中间阶段约为 10 μs，其中 **6 μs搬运时间被 Cube计算隐藏**。==
...
=={green}假设没有同步，可能出现**三类错误**。==

##### =={yellow}（1）数据还没搬完，Cube就开始计算==
...
> **=={yellow}消费者必须等待数据就绪，生产者必须等待缓冲区释放，后续阶段必须等待前置结果完整==。**
...
#### =={pink}5.4.6 第三道同步：完整Score生成后才能做Softmax==
...
### =={pink}5.5 为什么当前收益可能有限==

虽然 Tiling、双缓冲和流水并行能够隐藏部分数据搬运时间，但在当前算子的 `64×64` 小 Shape中，实际收益可能比较有限，主要有以下原因。

1. =={yellow}**只有两个 Chunk**，**可重叠的次数太少**==
2. =={yellow}存在**流水填充**和**排空开销**==
3. =={yellow}**搬运和计算时间**不一定**均衡**==
4. =={yellow}**小 Shape**下**固定开销占比较高**，双缓冲本身也有**控制成本**==，当能够隐藏的搬运时间很短时，新增的事件和切换开销会抵消一部分收益。
