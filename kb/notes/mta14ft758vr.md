...
> =={yellow}Traits可以理解为ROS的**编译期类型档案**。ROS**通过Traits判断**一个类型是不是消息==、类型名称是什么、长度是否固定、有没有Header，以及通信双方是否兼容。项目中我=={yellow}通过**偏特化**为所有**Protobuf**类型**补充了这些信息**，让ROS能够**把Protobuf类当作合法消息**处理==。
