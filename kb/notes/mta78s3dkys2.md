...
| =={yellow}精度== | **=={yellow}余弦相似度==** | **优化模型**与**基准模型 输出方向**和**=={yellow}整体分布===={yellow}**的==**===={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}一致性==** |
...
归一化后的16维特征经过一个 `Conv1d`，一次生成48维结果，再拆分成三组：
...
- **Q，Query，查询**：当前位置想寻找什么；
- **K，Key，键**：其他位置具有什么特征；
- **V，Value，值**：其他位置真正携带的信息。

可以把它类比成查资料：

> Q是我提出的问题，K是每本书的目录，先用Q和K判断哪本书相关，再从对应的V中读取实际内容。
...
一个注意力头相当于从一个角度观察信道数据；多个头可以同时学习不同类型的相关关系。
...
=={green}Kernel==：运行在NPU上的一段具体计算程序；GM：Device侧全局内存，容量较大，但访问成本高；

片上缓存：靠近计算单元、速度更快的存储空间；TransData：在不同数据排布格式之间转换；设备子图：ATC将完整模型拆分后，交给设备执行的一段计算图。

对于大模型，矩阵乘法可能占据主要时延；但SPF是一个小Shape、batch_size=1的轻量模型，单次矩阵计算规模不大，因此调度、格式转换和数据搬运的占比会更加明显。
