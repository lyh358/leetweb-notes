...
>            ▼
> ┌─────────────────────┐
> │ =={yellow}4.选择工作模式==      │→ =={yellow}基本定时== / =={yellow}输出比较== / =={yellow}PWMmaikuaitiaoz== / =={yellow}输入捕获==
> └──────────┬──────────┘
>            ▼
>      ┌─────┴─────┐
>      │需要中断？ │
...
3. 若=={yellow}参考电压是 ==`3.3V`===={yellow}，则 1 LSB 约等于 =={yellow}`3.3 / 4096 ≈ 0.8mV`=={yellow}，这叫=={yellow}**=={yellow}分辨率**===={yellow}。==
...
2. =={yellow}系统时钟 ==`SYSCLK`===={yellow}{yellow} 可以选择直接来自 HSI/HSE，也可以先经过 =={yellow}==**{yellow}PLL 倍频**=={yellow}=={yellow}{yellow} 后再作为主时钟==。
