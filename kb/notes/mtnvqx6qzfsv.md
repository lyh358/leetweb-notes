...
1. **=={yellow}核心机制：所有权==**
=={green}互斥锁有所有者概念==。=={green}任务调用==`xSemaphoreTake()`=={green}获取互斥锁==，=={green}获取成功之后，==**=={green}记录当前任务为锁的持有者==**；**只有**=={green}**持有者任务才可以调用**==`xSemaphoreGive()`=={green}**释放锁**，别的任务不能替它释放==。=={pink}这是**和二值信号量最大区别**，**信号量没有所有者**，**任意任务都可以 give**。==
2. **计数逻辑**
