...
=={green}无线信号从手机传到基站==时，=={green}会受到==建筑物反射、遮挡、多径和噪声等=={green}各种==影=={green}响==。=={green}基站需要根据SRS（===={green}Sounding Reference Signal，==**=={green}探测参考信号==**=={green}）、DMRS（===={green}Demodulation Reference Signal，==**=={green}解调参考信号==**=={green}）等参考信号，估计当前无线信道的状态，也就是信道矩阵 (H)。==
...
如果信道估计不准确，后续解调和预编码都会受到影响。

---

## 2. 为什么要引入AI信道估计

传统信道估计通常使用LS、MMSE和插值等数学方法。这些方法计算过程明确，但在大规模天线、多用户MIMO和复杂传播环境下存在一定局限：

- 固定算法对不同无线场景的适应能力有限；
- 参考信号稀疏时，难以恢复完整信道信息；
- 大规模天线之间存在复杂的空间和频率相关性；
- 不同场景可能需要人工设计不同的估计和滤波策略。

AI模型可以从历史信道数据中学习时间、频率和天线空间之间的相关性，从而得到更精细的信道估计结果。

但AI模型要真正进入基站，还必须解决一个问题：

> **模型不仅要算得准，还必须在基站物理层规定的时间内算完。**

因此，这段实习的重点不是训练一个新模型，而是把已有Transformer信道估计模型部署到Ascend 310P3，并优化它在真实NPU上的执行效率。

---

## 3. BESA_AI在基站系统中的位置

BESA_AI不是一个独立的AI应用，而是嵌入5G基站L1物理层的功能模块。
...
1. L1接收手机发送的SRS或DMRS参考信号；
2. 通过IPC把输入数据发送给BESA_AI；
3. AI模型在Ascend NPU上执行信道估计；
4. 输出信道矩阵 (H)；
5. 再通过IPC返回L1，供后续解调和预编码使用。
...
这段实习并不是优化整条链路的全部模型，而是以 **SPF_27B_10RB** 为主要对象，建立和验证部署侧优化方法。
...
## 6. 为什么模型已经能运行，还需要继续优化

SPF_27B_10RB最初能够成功转换并在310P3上运行，但仍存在明显的部署效率问题。
...
因此，优化目标不是简单减少MACs，而是同时减少：

> **冗余计算、Kernel调度、格式转换和全局内存搬运。**

---

## 7. 整体优化路线

这段实习采用的是“先建立基线，再逐层优化”的路线。
...
- Ascend 310P3板端推理；
- 基线性能和精度分析；
- 静态Shape、常量折叠和DCE计算图优化；
- ATC重新编译及融合效果验证；
- 时延由572 μs降至320 μs，降低44.1%；
- 余弦相似度保持0.999999；
- FP16、INT8及GELU替换方案评估和回退决策。
...
> 我在华为无线产品线做的是5G基站物理层AI信道估计模型的端侧部署与性能优化。简单来说，基站需要根据手机发送的SRS、DMRS参考信号估计无线信道矩阵，传统方法使用固定数学算法，而项目引入Transformer模型提升复杂场景下的估计能力。
> 
> 
...
我的岗位位于 **算法模型与端侧NPU硬件之间**，核心是端侧推理部署和性能优化，不是模型训练。
...
- 上游负责“模型应该怎么训练”；
- 我负责“模型怎样在310P3上正确、高效地运行”；
- 下游负责“怎样把优化后的模型集成进基站业务”。
...
# 三、SPF_27B_10RB模型端侧优化全过程

## 3.1 优化目标与约束

我实习期间始终围绕一项核心任务展开：

> 在不修改模型训练流程、不明显损失信道估计精度的前提下，降低SPF_27B_10RB模型在Ascend 310P3上的推理时延。
...
| --- | --- |
| OM稳态平均时延 | 572 μs |
| CPU ONNX平均时延 | 约7171 μs |
| OM端到端MSE | 约 (4.70\times10^{-7}) |
| OM端到端余弦相似度 | 0.999999 |

CPU与NPU的结果只能说明模型部署后的加速效果，真正用于后续优化对照的是同一310P3上的OM基线572 μs。
...
但这个模型的输入Shape固定为`[1,4,32,32,1]`，运行时不需要重新推导这些维度。
...
ATC通过匹配特定图结构实现算子融合。如果两个本来能够融合的计算节点中间插入Shape、Gather或Unsqueeze，融合规则就可能失效。

基线模型被编译成10个以上设备子图，子图之间需要额外调度和数据传递。
...
这是整段实习中已经完成实测闭环的核心优化。

### 3.4.1 优化思路

利用模型输入固定的特点，将运行时动态Shape推导提前到编译期完成，再删除无实际作用的节点。
