...
=={yellow}Protobuf，也就是==**=={yellow}Protocol Buffers==**=={yellow}，是一种===={yellow}Google 开源，==**=={yellow}跨语言、跨平台==**=={yellow}的===={yellow}**结构化数据定义**===={yellow}和===={yellow}**二进制序列化**=={yellow}=={yellow}框架===={yellow}。==
