...
在==={yellow}编译期（模板实例化阶段）=={yellow}，对于某个消息类型T，T在尝试使用实际类型代替偏特化****模板的形参的过程中（此时没有进入结构体内部代码），如果产生了非法类型/表达式，此时编译器不报编译错误，而是把这个模板特化版本从==={yellow}重载候选集=={yellow}里丢弃，编译器继续去找其它可行版本。
