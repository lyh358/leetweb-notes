...
=={yellow}你的项目用的是==**=={yellow}简单工厂==**（一个工厂类，一个 Create 函数，根据参数创建不同产品），面试说简单工厂就够，不要硬扯工厂方法 / 抽象工厂，你的项目没有用到。
...
工厂负责**生产对象**；多态负责**调用对象的行为**。
工厂产出基类指针，运行时多态调用子类的`Collect()`，两者配合工作。工厂解决对象创建，多态解决对象行为调用。

### Q：工厂模式一定需要继承、虚函数吗？

一般配合抽象接口 / 继承使用。如果没有统一基类，工厂返回不同类型对象，就没法统一保存到 vector 容器。你的项目中必须配合继承多态。

### Q：简单工厂的缺点？

如果产品越来越多，工厂内部的注册 / 判断逻辑会膨胀。但对于本项目采集器数量不多，完全够用。

### Q：是运行时生效还是编译期？

运行时，程序跑起来根据传入字符串动态创建不同对象。对比 SFINAE / 模板偏特化是编译期。
