...
1. **MTE1**：只做 L1 → L0A / L0B；给 Cube 准备输入矩阵数据；**不能直接访问 GM**docs.hisil...。
2. **MTE2**：读通路：GM/L2 → L1 / UB；把外部数据搬进 AI‑Core 内部存储；支持 ND↔NZ 格式转换鲲鹏昇腾开...。
3. **MTE3**：写通路：UB → L1 / L2 / GM；Vector 计算结果写回外部；支持 NZ↔ND 格式转换鲲鹏昇腾开...
