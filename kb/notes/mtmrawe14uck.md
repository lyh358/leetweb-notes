...
⑥ 调用 NvM，把 DTC、冻结帧存入 Flash/EEPROM =={green}**持久化**==，下电不丢失。
⑦ 通知 FIM 模块：故障发生，可做功能抑制（关闭某些功能）。

> ⚠️这里：**DEM 完成 DTC 生成、存储；DCM 此时完全没参与，没有报文发出**。故障存在 ECU 内部，**=={yellow}只有上位机主动读 UDS 才会把 DTC 发出去==**=={yellow}，不是故障一发生就自动上送 CAN 报文==。
...
2. **=={yellow}CanTp==**=={yellow}（ISO‑TP 网络层）==**=={yellow}接收 CAN 帧==**=={yellow}，==**重组完整 UDS 请求**=={yellow}=={yellow}，==**=={yellow}传给 DCM==**=={yellow}。==
3. **=={yellow}DCM 模块处理==**
...
**=={green}UDS：Unified Diagnostic Services，统一诊断服务==**=={green}。==它是=={yellow}**汽车诊断的应用层协议**===={green}，**定义一套标准化服务命令**==
