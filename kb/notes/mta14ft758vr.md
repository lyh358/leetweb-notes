...
=={yellow}在**编译期**（模板实例化阶段），对于某个**消息类型T**，T在尝试使用**实际类型**代替偏**特化模板的形参**的过程中（此时没有进入结构体内部代码），如果**产生了非法类型**/表达式，此时编译器**不报编译错误**，而是把这个**模板特化版本**从**重载候选集**里**丢弃**，编译器继续去**找其它可行版本**。==

=={yellow}如果==**=={yellow}所有候选全部替换失败==**=={yellow}，最后==**=={yellow}才报编译错误==**=={yellow}。==
...
> std::=={yellow}**is_base_of<Base, T> ::value**==
> std::=={yellow}**enable_if<condition>::type**==

其中：

- `std::is_base_of`**=={yellow}判断T===={yellow}是否继承某个基类==**；
- `std::enable_if`=={yellow}**只在条件成立时提供一个有效的**==`type`。
...
template<typename T>
struct IsMessage<T,typename enable_if<is_base_of_v<google::protobuf::Message, T>>::type> : boost::true_type {};
```
