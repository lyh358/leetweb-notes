...
修改某一位时，要避免影响其他已经配置好的位。

- **底层实现原理**

1. 置位 / 清零属于**按位操作**，只修改目标位，其他位保持不变。
...
面试里常用来考察你对上电后程序是怎么跑起来的是否真正理解。

也经常延伸到 BootLoader、向量表、中断初始化这些问题。

- **底层实现原理**

1. STM32 上电或复位后，内核先从固定启动地址取出向量表中的前两个值。
2. 第 1 个值加载到 MSP，作为主堆栈指针；第 2 个值加载到 PC，跳到 `Reset_Handler` 执行。
...
STM32 定时器常用来做 定时中断、PWM 输出、输入捕获、计数。

本质就是先让定时器按设定频率计数，再决定什么时候触发事件。

- **底层实现原理**

1. 先开启 TIM 外设时钟，否则定时器寄存器不工作。
2. 配置预分频器 PSC 和自动重装值 ARR，决定计数频率和溢出周期。
3. 根据需求选择模式，比如 基本定时、中断、输出比较、PWM、输入捕获。
4. 如果要定时中断，还要配置 DIER、NVIC，让更新事件能进入中断函数。
5. 最后使能定时器 CEN，计数器开始运行，到达条件后产生更新事件或输出波形。
...
4. ARM 架构里定义了**寄存器组、异常 / 中断机制、堆栈模型、权限级别**，STM32 则在这个基础上集成了 GPIO、USART、TIM、ADC 等片上外设。
...
>
> 通过 PC 反查代码：拿到 PC 值，结合 map 文件找函数，再看反汇编，映射到 C 源码行。=={yellow}=={yellow}=={pink}=={pink}=={pink}重点内容==========