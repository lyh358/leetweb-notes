...
=={pink}5.3 UART DMA 接收（ESP32 高性能方案）==**=={green}GPS 字节流入 UART 硬件==** =={green}FIFO，DMA 硬件自动把字节搬运到内存缓冲区，==**=={green}不需要 CPU 参与搬运==**=={green}。==DMA 完成 / 半满 / 超时，才产生一次中断通知 CPU。CPU 一次拿到一大段 GPS 数据。
