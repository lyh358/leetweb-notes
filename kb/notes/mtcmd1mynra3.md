...
┌─────────────────────────────────────────────────────────────┐
│ CollectorFactory【简单工厂类】静态工厂                        │
│ 静态方法：static unique_ptr<IMetricCollector> Create(string name)
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│ MonitorAgent【主业务类】                                      │
│ 私有成员：std::vector<std::unique_ptr<IMetricCollector>> collectors;
└─────────────────────────────────────────────────────────────┘
```

#### 归属 & 所有权（重点面试）

1. **`IMetricCollector`**：抽象接口，定义所有采集器统一行为；**没有实例，只做接口契约**。
2. **CpuCollector/MemoryCollector…**：具体采集实现子类，继承`IMetricCollector`；**对象在堆上 new 出来**。
3. **CollectorFactory**：**静态简单工厂类，不持有任何采集器对象，只负责创建对象，不拥有对象所有权**。工厂函数返回`std::unique_ptr<IMetricCollector>`，把堆对象的**独占所有权转移出去**。
4. **MonitorAgent**：**采集器对象的唯一拥有者**。Agent 内部`vector<unique_ptr<IMetricCollector>> collectors`接管全部采集器的所有权；Agent 析构的时候，vector 销毁，全部采集器自动释放，无内存泄漏。
...
1. =={yellow}写==`DiskCollector`===={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}**继承**=={yellow}`IMetricCollector`=={yellow}，实现 Collect ()==
...
> =={green}工厂模式把对象创建和对象使用分开。Agent只向工厂传入==`cpu`=={green}、==`memory`===={yellow}={yellow}={yellow}={yellow}={green}等名称，工厂负责返回对应的采集器。后续增加新指标时，只需要实现接口并注册到工厂，不需要修改Agent的核心采集循环。==
...
### =={pink}4.1 致命依赖与职责分离：ROS Topic 强依赖 roscore==
...
- **=={green}ROS Topic：机器人业务域==**，负责感知、定位、规划、控制这些业务模块之间交互。
- **=={green}gRPC 监控链路：基础设施域==**，负责机器硬件资源状态，属于运维监控。
...
=={yellow}节点性能指标是**周期性**产生的**连续数据**==。gRPC支持在一次RPC中持续发送多条消息，=={yellow}**适合**==这种监控场景。

所以这里=={green}不是因为ROS不能跨机器通信==，而是：=={yellow}ROS Topic 是==**=={yellow}异步发布‑订阅，发完即忘，没有应答语义，==****=={yellow}不知道 Center 有没有收到这条监控消息==**=={yellow}，没有 ACK 确认。==

> gRPC与Protobuf结合自然，并且提供了清晰的客户端流、服务端流和请求响应接口，更适合独立的集中监控链路。

---

## =={pink}5. Agent和Center分别负责什么==
...
1. =={yellow}**接收**==不同Agent=={yellow}**上报**==的指标；
2. 根据`node_id`=={yellow}**区分**==不同计算=={yellow}**节点**==；
3. =={yellow}**缓存**==每个节点的最新=={yellow}**性能快照**==；
4. 根据CPU和内存阈值=={yellow}**判断健康状态**==；
5. =={yellow}向**监控端**持续**推送**==最新指标。
...
`stream NodeMetrics`表示=={yellow}客户端可以在**一次RPC调用中连续发送多条**==`NodeMetrics`。
...
如果每=={yellow}**产生一次指标就创建一次普通RPC**，会**不断重复请求建立和结束**过程==。

客户端流允许Agent建立**=={yellow}一次调用后持续发送==**数据，更符合“节点不断上报监控指标”的场景。
...
=={yellow}**Qt监控端需要持续获得**最新指标。如果使用**普通请求响应**，就需要**反复轮询Center**。==

服务端流允许监控端发送=={yellow}**一次订阅请求后**，由Center**持续推送**数据，更**适合实时**监控界面==。
