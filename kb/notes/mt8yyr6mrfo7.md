...
- **=={yellow}工程侧==**=={yellow}：把PC端算法迁移成边缘侧可在昇腾板端能够运行的推理系统。==
...
**=={yellow}原始充放电数据 → 数据清洗与标准化 → 11维退化特征提取 → GRA筛选强相关特征 → PCA压缩为4维健康因子 → LSTM容量预测 → EOL与RUL判断==**
...
### =={yellow}2. 数据预处理（删重+补缺+提取特征+特征标准化）==

论文实验中的主要预处理包括：

- =={green}**删除重复**的充电记录==；
- =={green}对**缺失**或**无效数据**进行检查==，并使=={green}用**对应特征的均值**进行补充==；
- 按**充放电循环原始数据**=={yellow}整理==按维度的电压、电流、温度和**容量**和**特征数据**；
- =={green}对**提取后的特征和容量**进行Z-score**标准化**。==
...
### =={pink}4. 为什么提取物理特征，而不是直接输入原始曲线==

NASA电池数据规模相对有限。如果直接=={yellow}使用完整采样曲线，模型**输入维度会明显增加**==，而且=={yellow}不同循环的**序列长度不一致**==，需要进行重采样或填充。

=={yellow}提取==充电时间、电压下降、平均温度等=={yellow}特征==，=={yellow}可以==把每个充放电循环=={yellow}转换为**固定维度向量**==，同时保留一定的物理意义，=={yellow}**减少模型规模**和小样本下的**过拟合风险**，更**适合**后续**边缘端部署**==。

它的局限是依赖人工特征设计，可能损失原始曲线中的局部细节。如果数据规模足够大，可以进一步尝试1D CNN、TCN或Transformer直接处理原始时序曲线。

---

## =={pink}A2：GRA特征筛选与PCA降维：优化模型输入，构建健康因子==

### =={green}1. 为什么不能把11维特征全部输入LSTM==

11个特征虽然都来自充放电过程，但=={yellow}它们对容量退化的**敏感程度不同**==，而且=={yellow}部分特征之间可能**表达相似信息。**==

如果直接=={yellow}全部输入模型==，可能出现：

- =={yellow}**弱相关特征**引入**噪声**==；
- =={yellow}**高相关特征**之间存在**重复信息**==；
- =={yellow}增加**模型参数**和**训练难度**==；
- =={yellow}影响==模型在寿命=={yellow}**中后期**==对=={yellow}关键退化趋势的**判断**==。

因此，我=={pink}采用**GRA**和**PCA**分两步**优化输入**。==

---

### =={green}2. GRA：筛选与容量退化强相关的特征==

=={yellow}GRA，也就是灰色关联分析==，通过=={yellow}比较**特征序列**与**容量序列**随循环变化的**趋势相似程度**，**衡量**每个**特征**与**容量退化**的**关联性**。==
...
=={yellow}项目以**0.75作为筛选阈值**，从11维特征中**保留9维**==，剔除了：
...
### =={green}3. PCA：进一步压缩冗余信息==

经过=={yellow}GRA筛选后，剩余**9个特征**都与**容量退化**具有较**强关联**==，但=={pink}它们之间仍然可能存在**信息重叠**==。例如平均放电电压、放电电压下降量和恒流放电时间，都可能从不同角度描述相近的退化现象。

因此，进一步使用=={yellow}PCA将**相关特征  投影** 到**相互正交**的**主成分方向**。==
...
项目中**前***_**四个主成分**的_*累计方差贡献率**达到**98.65%**，因此将9维特征**压缩成4维健康因子=={yellow}，{yellow}作为**LSTM的输入。**==

---

GRA和PCA=={green}解决的不是同一个问题==：

- =={green}GRA利用容量标签==，回答“=={yellow}哪些特征**与容量退化更相关**==”；
- PCA=={green}不使用==容量标签，回答“=={yellow}如何**消除**入选特征之间的**冗余和共线性**==”。
...
电池在=={yellow}**当前循环的健康状态**不仅与**当前采集值**有关，还与**此前多个循环**形成的**长期退化趋势**有关，因此这是一个**典型的时序预测问题**。==

=={yellow}普通RNN能够处理序列==，但在=={yellow}**较长序列反向传播**过程中容易出现**梯度消失**或**梯度爆炸**==，=={yellow}**难以稳定保留**较早循环的信息。==

=={yellow}LSTM在**RNN基础**上增加了**细胞状态**以及**遗忘门**、**输入门**和**输出门**：==

- =={green}遗忘门==决定**保留多少历史信息**；
- =={green}输入门==决定**写入多少当前信息**；
- =={green}输出门==决定**向下一时刻输出多少状态**。

这种门控结构能够=={yellow}**缓解**长序列中的梯度问题==，=={yellow}更适合学习电池**长期、非线性**的容量退化规律==。这里应表述为“缓解”，不能说LSTM完全解决了梯度消失和梯度爆炸。

---

### =={pink}2. 模型输入与输出==

经过GRA和PCA处理后，每个充放电循环被表示为**=={yellow}4维健康因子==**。
...
=={yellow}模型直接预测的是**容量**==，而不是直接输出“还剩多少寿命”。
...
> =={yellow}先完成**容量回归**，再根据容量预测曲线**推导EOL和RUL**。==

### 3. 训练方法

论文研究阶段采用的主要设置包括：

- 优化健康因子模型使用=={yellow}50个LSTM神经元==；
- 最大=={yellow}训练轮数为200==；
- 使用=={yellow}Adam优化器==；
- 初始**学习率为0.01**；
- 使用**MSE作为训练损失**；
...
论文中比较了21步和42步两种预测策略。这里的“=={yellow}21步、42步”主要表示**预测跨度**==，即=={yellow}使用当前及历史信息**预测若干循环之后的容量**==。

=={yellow}预测**跨度越大**，提前量越高==，但=={yellow}**误差也更容易累积**==；=={yellow}预测**跨度越小**，能够更**细致地跟踪局部退化**变化。==

需要注意，论文中的=={yellow}“预测跨度21步”和部署模型**输入Shape中的“序列长度21”**==是两个概念：

- =={green}预测跨度表示**向未来预测多少个循环**；==
- =={green}输入序列长度表示**每次模型推理使用多少个历史循环**。==
...
- =={yellow}**一维**综合健康因子==＋LSTM；
- =={yellow}**GRA＋PCA**构造的**多维**优化健康因子==＋LSTM。

目的在于=={green}验证预测**提升来自特征优化**==，而不只是LSTM本身。
...
该指标直接反映预测**=={yellow}寿命终点与真实寿命终点相差多少个循环==**，比单纯使用全周期容量误差更贴近寿命预测目标。
...
=={yellow}首先，我对==**=={yellow}NASA电池充放电数据集==**=={yellow}数据进行==**=={yellow}清洗和标准化==**=={yellow}，并从充电、放电和温度变化中==**=={yellow}提取了11维退化特征==**=={yellow}。因为这些==**=={yellow}特征对容量衰减的敏感程度不同==**=={yellow}，所以我==**=={yellow}先使用GRA===={yellow}计算==**=={yellow}每个特征与容量退化趋势的==**=={yellow}关联程度==**=={yellow}，以==**=={yellow}0.75为阈值==**=={yellow}保留==**=={yellow}9个强相关特征==**=={yellow}。==

=={yellow}但是GRA只能剔除弱相关特征，不能解决入选==**=={yellow}特征之间的信息冗余==**=={yellow}，因此我又使用==**=={yellow}PCA进行降维==**=={yellow}。====={yellow}**=={yellow}**==**{yellow}************************************************************=={yellow}************************************==********{yellow}************************=={yellow}************{yellow}****={yellow}前四个主成分累计方差贡献率达到98.65%******==******{yellow}{yellow}******{yellow}******=={yellow}**{yellow}，所以最终将9维特征**=={yellow}**=={yellow}**压缩成4维健康因子**==****。

=={yellow}之后，我==**=={yellow}把连续多个循环的健康因子输入LSTM==**=={yellow}，通过LSTM学习电池长期、非线性的容量退化规律，再==**=={yellow}通过全连接层输出容量预测值==**=={yellow}。模型不是直接输出RUL，而是==**=={yellow}先得到未来容量曲线==**=={yellow}，====={yellow}={yellow}=={yellow}******************************************************************=={yellow}**{yellow}**************************************=={yellow}******{yellow}********************=={yellow}******{yellow}****={yellow}再根据EOL阈值找到容量首次越过阈值的循环位置**==****{yellow}****************************=={yellow}**********{yellow}**********，=={yellow}**************=={yellow}**{yellow}**=={yellow}={yellow}进而计算剩余寿命**=={yellow}****=={yellow}。==

=={yellow}实验设计上，我==**=={yellow}比较了一维综合健康因子和多维优化健康因子==**=={yellow}，也==**=={yellow}比较了21步和42步两种预测策略==**=={yellow}，用来验证特征优化和预测跨度对结果的影响。==**=={yellow}评价时==**=={yellow}不仅看==**=={yellow}RMSE和MAE==**=={yellow}，还单独分析==**=={yellow}EOL循环误差==**=={yellow}，因为全周期容量拟合准确，并不代表寿命终点一定判断准确。==
...
=={green}算法侧解决的是“如何预测”==，=={yellow}工程部署侧解决的是“如何让训练完成的模型在昇腾边缘设备上运行”。==

整体部署链路为：

**=={yellow}PyTorch ==`.pth`===={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}模型 → ONNX中间模型 → ATC编译 → OM离线模型 → ACL Lite板端推理 → 结果反标准化与验证==**

这个过程中需要=={green}解决四类问题==：

- =={yellow}PyTorch**训练模型**如何**转换**为跨框架的**静态计算图**；==
- =={yellow}**LSTM动态序列**如何**适配**NPU的**静态编译**；==
- =={yellow}**ONNX**模型如何**编译为**Ascend 310B4能够执行的**OM模型**；==
- =={yellow}如何在ARM64 Linux**板端**建立完整的**推理和资源管理流程**。==
...
`.pth`=={yellow}通常保存的是**PyTorch模型权重**==，也就是每一层的参数，并不等于一个可以被昇腾NPU直接执行的模型。
...
如果=={yellow}直接在ARM板端使用PyTorch加载==`.pth`=={yellow}，即使能够运行，也主要是**在CPU上执行**，**不能使用**Ascend **NPU的推理能力**==，而且板端还要安装完整的PyTorch依赖。
...
=={green}如果网络结构、隐藏层大小或者层数与训练时不一致==，`load_state_dict()`=={green}会出现**参数Shape不匹配**；即使通过部分加载，也可能导出一个**语义错误的模型**。==
...
- =={yellow}第一个维度是Batch Size；==
- =={yellow}21表示历史序列长度；==
- =={yellow}4表示PCA处理后的健康因子维度；==
...
=={yellow}固定Shape以后，ATC可以在**编译阶段提前确定  中间张量的大小和形状**，**内存规划布局**，**算子执行方式**，有利于**编译期自动优化**：==
...
=={green}动态Shape灵活性更高，但会增加**编译和运行时推导开销，**==如果**输入维度是动态的，芯片就无法提前规划**，而且LSTM动态序列在不同CANN版本中的支持程度存在差异。
...
2. =={yellow}检查==**=={yellow}输入输出名称、Shape和数据类型是否一致==**=={yellow}；==
...
=={yellow}不同Ascend芯片**支持的算子**、**AI Core架构**和**内存结构**可能**不同**。==
...
**=={yellow}回归任务精度优先且为软实时任务==**：电池容量预测属于连续值回归，较小的数值误差也可能影响EOL越阈位置。
...
=={yellow}普通前馈网络通常是**一次输入对应一次输出**，每次**推理之间相互独立**==。

=={green}LSTM除了**当前输入**(x_t)，还依赖**上一时刻**的：==

- =={green}**隐藏状态**(h_{t-1})；==
- =={green}**细胞状态**(c_{t-1})。==

=={yellow}**在PyTorch中**，**这些状态**可以在Python运行逻辑中**动态创建和传递**==；=={yellow}但**OM是静态计算图**，每次模型调用都是**一次固定的输入到输出映射**==，不会自动保存Python对象中的状态。

=={green}如果不做处理，**每次调用模型**都会**从零状态开始**，**跨次推理**的**时序上下文**可能丢失。==
...
```undefined
y  ：容量预测值
hn ：本次推理后的隐藏状态，可以理解为当前时刻对外输出的记忆表征
cn ：本次推理后的细胞状态，可以理解为长期记忆主通道
```
...
=={yellow}板端使用**长度为21的队列**保存**最近的健康因子**==：
...
=={yellow}每次加入新的健康因子，同时**移除最早的数据**，再将窗口整理为：==

[ [1,21,4] ]

**=={yellow}通过固定滑动窗口模拟LSTM长期时序记忆==**

这种方式满足了OM模型的固定Shape要求，也能够让每次推理看到最近一段退化历史。

---

### =={pink}5. 为什么不直接使用单个时间步==

=={yellow}如果每次只输入当前一个循环的4维健康因子，那么模型只能依赖==`h/c`=={yellow}状态保存历史信息。==
...
=={yellow}OM只是编译完成的模型文件，还不能直接运行。==真正完成板端部署还需要处理：
...
因此，=={yellow}板端部署的交付物不仅是一个OM文件，还包括**围绕OM模型建立的推理程序**。==

---

### 3. 为什么使用ACL Lite

=={yellow}ACL是昇腾平台底层**推理接口**==，可以控制：
...
=={yellow}ACL Lite是对ACL常用流程的Python封装==。项目=={yellow}使用它**简化资源初始化、模型加载和执行流程**==，但=={yellow}底层实际推理仍由ACL Runtime完成==。

因此，准确口径是：

> =={yellow}我基于ACL Lite搭建了板端推理工程，而不是自己重新实现了ACL运行时。==
