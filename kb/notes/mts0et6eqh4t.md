...
- 增加**=={yellow}计算图复杂度==**，基线模型被编译**=={yellow}**成==**==={yellow}10个以上设备子图(CPU fallback+GM搬运+kernel调度=={yellow}****=={yellow})=={yellow}**，基线模型得到的静态计算图虽然**=={yellow}能运行=={yellow}**=={yellow}，但是并**===={yellow}不干净==**；
...
主要问题是存在**大量不必要的动态shape推导算子**，将**计算图切的很细碎**（10个子图），导致**ATC自动融合失败**和**额外kernel启动调度**还有**数据搬运**；=={yellow}（====**={yellow}计算图分析**=={yellow}=={yellow}）==
