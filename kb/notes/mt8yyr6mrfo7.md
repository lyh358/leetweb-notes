...
- **=={yellow}删除重复==**的充电记录；
- 对**=={yellow}缺失==**=={yellow}或==**=={yellow}无效数据==**进行检查，并使用对应**=={yellow}特征的均值==**=={yellow}进行==**=={yellow}补充==**；
...
**LSTM**在**=={yellow}RNN基础上==**=={yellow}增加了==**=={yellow}细胞状态==**=={yellow}以及==**=={yellow}遗忘门==**=={yellow}、==**=={yellow}输入门==**=={yellow}和==**=={yellow}输出门==**=={yellow}：==
