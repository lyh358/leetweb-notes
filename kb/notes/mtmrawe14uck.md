...
- **DEM(Diagnostic Event Manager)**：BSW 模块，事件去抖、确认、Event‑DTC 映射、存储冻结帧、NvM 持久化；**DTC 真正的管家**
- **DCM(Diagnostic Communication Manager)**：BSW 诊断通信，UDS 协议解析；**不和 ASW 直接打交道，只和 DEM 交互**
...
4. DEM 返回结果给 DCM，DCM 回复`0x54`肯定响应给上位机CSDN博...。
