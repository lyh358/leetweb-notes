...
> STM32‑TIM 启动流程
> ┌─────────────────────┐
> │ =={yellow}1.开启TIM外设时钟==   │←【必须第一步，无时钟寄存器不工作】
> └──────────┬──────────┘
>            ▼
> ┌─────────────────────┐
> │ =={yellow}2.配置 PSC 预分频器== │→ 设置计数器计数=={yellow}频率==
> └──────────┬──────────┘
>            ▼
> ┌─────────────────────┐
> │=={yellow}3.配置 ARR 自动重装值==│→ 设置溢出/=={yellow}计数周期==
> └──────────┬──────────┘
>            ▼
> ┌─────────────────────┐
> │ =={yellow}4.选择工作模式==      │→ =={yellow}基本定时== / =={yellow}输出比较== / =={yellow}PWM脉宽调制== / =={yellow}输入捕获==
> └──────────┬──────────┘
>            ▼
>      ┌─────┴─────┐
>      │=={pink}需要中断？== │
...
>          ▼
> ┌─────────────────────┐
> │ =={yellow}6.置位 CEN 使能定时器==│←【最后一步，启动计数器】
> └──────────┬──────────┘
>            ▼
> ┌─────────────────────┐
> │ 计数器开始持续计数  │
...
2. 普通任务运行在用户态，只能访问被允许的代码区、数据区和外设；涉及中断控制、任务切换、时钟配置这类敏感操作时，必须通过=={yellow}{pink} ====={pink}**={pink}SVC 系统调用**=={yellow}=={pink}进入内核态==。
...
1. =={yellow}外设产生中断请求==后，先=={pink}送到 ====={pink}**={pink}NVIC**=={yellow}，NVIC 判断该中断=={pink}是否使能=={yellow}、=={pink}优先级是否满足===={green}响应条件。==
...
1. STM32 用**=={yellow}定时器==**=={yellow}产生固定频率的 PWM 波，核心参数是 ==`PSC`===={yellow}={yellow}、=={yellow}`ARR`=={yellow}、==`CCR`。
...
3. =={green}对别名地址写 ==`0/1`===={yellow}={green}，就等价于把目标位清 0 或置 1==；读别名地址，就能直接得到该位的值。
...
- =={pink}普通方式是 ====={pink}**={pink}按位运算改寄存器**==
- =={pink}位带方式是 ====={pink}**={pink}直接把这一位当变量访问**==

## 13 STM32 的 ADC 精度

- **应用场景**

=={yellow}ADC 精度决定了 ====={yellow}**={yellow}模拟量转数字量时能分多细**==，直接影响电压、温度、电流等采样结果的分辨能力。

面试里一般既会问 **=={yellow}分辨率==**=={yellow}，也会追问 ====={yellow}**={yellow}实际精度是否真的能达到标称值**=={yellow}=={yellow}。==
...
3. 若=={yellow}参考电压是 ==`3.3V`===={yellow}={yellow}，则 1 LSB 约等于 =={yellow}`3.3 / 4096 ≈ 0.8mV`=={yellow}，这叫=={yellow}**=={yellow}分辨率**===={yellow}。==
...
=={yellow}时钟树用来给 ====={yellow}**={yellow}CPU、总线、外设**=={yellow}=={yellow}{yellow} 分配不同频率的时钟。==

面试问它，本质是看你是否清楚=={yellow}{green} ====={green}**={green}系统时钟从哪来、怎么分给各模块**==**。**

- **底层实现原理**

1. =={pink}STM32 的时钟源一般有 ====={pink}**={pink}HSI、HSE、LSI、LSE**=={yellow}，=={green}高速时钟给系统和外设==，=={green}低速时钟常给 RTC、看门狗==。
2. =={yellow}系统时钟 ==`SYSCLK`===={yellow}={yellow}{yellow} 可以选择直接来自 HSI/HSE，也可以先经过 =={yellow}==**{yellow}PLL 倍频**=={yellow}=={yellow}{yellow} 后再作为主时钟==。
...
1. =={pink}STM32 的内存一般包括 ====={pink}**={pink}Flash、SRAM**==，部分型号还带 **CCM、Backup SRAM、EEPROM 模拟区** 等。
...
3. STM32=={yellow}{pink} 复位后虽然是从 ==`0x00000000`===={yellow}={yellow}{pink} 开始取向量表，但芯片会把启动区域=={yellow}=={pink}**重映射**===={pink}到这个地址==。
...
- **应用场景**
=={yellow}STM32 串口接收常见有 ====={yellow}**={yellow}轮询、中断、DMA**=={yellow}=={yellow}{yellow} 三种方式。==
本质区别在于：**=={pink}CPU 参与程度不同，实时性和效率也不同==**。
- **底层实现原理**

1. **=={green}轮询==**=={green}：CPU 一直检查接收标志位 ==`RXNE`===={yellow}={green}，有数据就读寄存器==；实现简单，但 CPU 会一直空转，效率最低。
2. **=={green}中断==**=={green}：收到数据后串口触发中断，CPU 跳到 ISR 里读 ==`DR`；比轮询省 CPU，适合低速或不连续数据。
3. **=={green}DMA==**=={green}：串口收到数据后，由 DMA 自动把 ==`DR`===={yellow}={yellow}{green} 搬到内存==，CPU 不用逐字节参与；适合高速、大量、连续接收。
...
2. 配置入口本质都在=={yellow}{pink} ====={pink}**={pink}RCC 时钟控制模块**==，核心寄存器一般是 `RCC->CFGR`。
...
1. STM32 的=={yellow}{pink} Flash 是按 ====={pink}**={pink}页（Page）或扇区（Sector）**=={yellow}=={yellow}{pink} 管理==的，不是按字节独立擦除。
...
2. =={green}如果是 ====={green}**={green}轮询**=={yellow}=={yellow}{green} 或 ====={green}**={green}中断**=={yellow}=={yellow}{green} 方式，CPU 再去读取 DR，读出来后一般存到变量、数组或缓冲区里==。
3. =={green}如果是 ====={green}**={green}DMA**=={yellow}=={yellow}{green} 方式，DMA 会自动把 DR 里的数据搬运到 RAM 中指定的数组或缓存区==。
...
1. 常用=={pink}在线调试接口是 ====={pink}**={pink}SWD 和 JTAG**==，STM32 最常用 SWD，占用引脚少。
...
3. 关键是=={pink}取出异常现场里的 ==`PC`===={yellow}={pink}，它通常指向出错指令附近==。
