...
> 这项功能主要服务产线和硬件调试。PC1300内部有很多模拟和数字电压通道，以前测试时需要人工修改寄存器。=={yellow}**我在PTC协议里增加了ANA和DC两条扩展命令**==，=={yellow}**上位机传入通道号后**==，=={yellow}**MCU通过CDD_SPAD**读改写对应MUX寄存器==，=={yellow}把目标电压切到测试引脚==，=={yellow}再复用GetADC读取滤波后的电压==，=={yellow}由上位机和阈值表比较==。这样就把手工寄存器操作变成了标准化、可自动执行的测试流程。
