...
1. =={yellow}**检查**ONNX**输入输出名称**、**Shape**和**数据类型**和**数据布局**；==
...
### =={pink}3.3.2== =={pink}分析工具（Netron、ATC、msprof、ACL推理程序）==
...
使用**CANN提供**的msprof采集模型执行过程，重点关注：
...
主要问题是存在**大量不必要的动态shape推导算子**，将**计算图切的很细碎**，导致**ATC自动融合失败**和**额外kernel启动调度**；=={yellow}（===={yellow}计算图分析）==

同时还有太多**Cube和Vector频繁切换导致**的**中间张量的GM和片内缓存搬移**，以及**transData开销**。（NPU执行分析）
...
### =={pink}3.4.3 为什么没有直接使用现成简化工具==

实际环境中遇到了几个问题：

- Windows版`onnx-simplifier`发生崩溃；
- =={yellow}ARM服务器无法联网安装==缺失依赖；
...
5G基站L1信道估计场景下，=={green}算法优化几乎无操作空间==：

**精度是硬约束**：5G L1对信道估计精度有严格协议要求，任何数学不等价的近似都有可能导致系统级性能降级**模型已很小**：4层Transformer、32×32矩阵，**没有冗余可剪****维度由协议决定**：slot数、RB数、天线数都是5G标准规定的，不可随意缩减**ATC融合优先**：即使找到数学等价的替换方案，如果破坏了ATC的融合模式，也会像P1一样反向优化

---

## =={pink}3.6 第五阶段：FP16/32混合精度与INT8量化评估==
...
=={yellow}FP16方案没有稳定性能收益==，=={yellow}INT8链路受精度限制太大，组内不建议使用==，工具和编译器限制，均未进入最终版本。
