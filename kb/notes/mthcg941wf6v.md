...
1. **=={yellow}ARM A55 8 核==**：=={green}分为**控制 CPU**（操作系统、驱动）、**AI‑CPU：跑不适合 AI‑Core 的算子**==（少量标量、复杂逻辑算子）。
2. **=={yellow}AI‑Core Cluster：8 个 AI Core==**=={yellow}，==达芬奇 V200，推理计算核心，矩阵 / 向量 / 标量计算。
3. **=={yellow}TS 任务调度器 Task Scheduler==**：硬件任务调度，把算子任务分发到各个 AI Core。
...
**Cube 只能直接访问 L0A、L0B、L0C**；不能直接访问 L1、UB。L1 的数据必须 MTE1 搬到 L0A/L0B 才能给 Cube 计算。
**Vector 只能直接访问 UB**，向量运算必须把数据放进 UB。Vector 不能直接读写 L1/L0 系列 BufferCSDN博...。
L0C 是 Cube 输出结果缓存；L0C 不能被 MTE 直接读，L0C 结果走 FixPipe 输出到 UB 或者 L1

## MTE 分工背诵

1. **MTE1**：只做 L1 → L0A / L0B；给 Cube 准备输入矩阵数据；**不能直接访问 GM**。
2. **MTE2**：读通路：GM/L2 → L1 / UB；把外部数据搬进 AI‑Core 内部存储；支持 ND↔NZ 格式转换。
3. **MTE3**：写通路：UB → L1 / L2 / GM；Vector 计算结果写回外部；支持 NZ↔ND 格式转换。
