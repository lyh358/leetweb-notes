...
2. 第=={yellow}1 个值加载到MSP==，作为主堆栈指针；第 =={pink}2 个值加载到PC==，跳到`Reset_Handler`执行。
3. `Reset_Handler` 里会完成=={green}运行时初始化==，比如把 Flash 中的 `.data` 拷贝到 RAM、把 `.bss` 清零。
4. 然后执行`SystemInit()`=={pink}，==完成时钟、FPU、向量表偏移等==底层初始化。
...
本质就是先让=={green}定时器按设定频率计数，再决定什么时候触发事件==。
...
> STM32‑TIM 启动流程
> ┌─────────────────────┐
> │ 1.开启TIM外设时钟   │←【必须第一步，无时钟寄存器不工作】
> └──────────┬──────────┘
>            ▼
> ┌─────────────────────┐
> │ 2.配置 PSC 预分频器 │→ 设置计数器计数频率
> └──────────┬──────────┘
>            ▼
> ┌─────────────────────┐
> │3.配置 ARR 自动重装值│→ 设置溢出/计数周期
> └──────────┬──────────┘
>            ▼
> ┌─────────────────────┐
> │ 4.选择工作模式       │→ 基本定时 / 输出比较 / PWM / 输入捕获
> └──────────┬──────────┘
>            ▼
>      ┌─────┴─────┐
>      │需要中断？ │
...
2. 配置=={pink}预分频器 PSC==和=={pink}自动重装值 ARR==，决定计数=={green}频率==和溢出=={green}周期==。
...
一种用数字信号模拟模拟量的技术。通过快速切换高低电平，改变占空比，来控制平均电压或功率。
**TIM（Timer，定时器）**
硬件外设，产生 PWM 的源头。
**PSC（Prescaler，预分频器）**
降低计数器时钟频率的除法器。
...
1. STM32 内核大多基于=={yellow}ARM Cortex‑M 架构==，属于面向微控制器的 =={pink}RISC 体系==，特点是指令精简、执行效率高。
2. =={pink}Cortex‑M 采用===={pink}**哈佛结构**=={yellow}思想，=={pink}指令=={yellow}和=={pink}数据访问==通路=={pink}分离==，所以取指和访存效率更高。
...
5. 所以可以理解为：=={yellow}ARM Cortex‑M 提供通用计算核心，STM32 在外面封装了存储器和各种外设，最终形成完整单片机。==
...
在 MCU 上=={green}做 RTOS / 嵌入式 OS 时==，如果既想=={pink}让应用任务能跑，又不想它随便改内核数据==、乱配外设，就要区分**=={green}用户态和内核态==**。
...
2. 普通任务运行在用户态，只能访问被允许的代码区、数据区和外设；涉及中断控制、任务切换、时钟配置这类敏感操作时，必须通过=={yellow}{pink} ====**={pink}SVC 系统调用**=={yellow}=={pink}进入内核态==。
...
| 概念 | 一句话 |
| --- | --- |
| **Thread mode** | 正常执行用户代码/RTOS任务的状态，Privileged/UnPrivileged |
...
| **MemManage Fault** | =={green}违反 MPU 规则（如越权写只读区）时触发的异常== |
| **HardFault** | =={green}最高优先级异常==，所有其他异常无法处理或配置错误时的=={green}兜底== |
...
中断用于**=={pink}外设事件来了立即处理==**，避免 CPU 一直轮询浪费时间。

常见场景有=={green}按键触发==、=={green}串口收发==、=={green}定时器到时==、=={green}ADC 转换完成==。

- **底层实现原理**

1. =={yellow}外设产生中断请求==后，先=={pink}送到 ====**={pink}NVIC**=={yellow}，NVIC 判断该中断=={pink}是否使能=={yellow}、=={pink}优先级是否满足===={green}响应条件。==
2. 如果=={pink}可以响应==，=={pink}CPU 会暂停当前任务==，并把当前=={pink}现场自动压栈保存==，主要包括 PC、LR、xPSR、R0‑R3、R12。
3. 然后 CPU 根据**=={pink}中断向量表==**取出对应中断服务函数入口地址，=={pink}跳转执行 ISR==。
4. =={pink}ISR 中==一般=={green}先判断中断源，再处理事件，最后清除中断标志==，=={yellow}否则可能会重复进入中断==。
5. ISR 执行结束后，CPU 通过异常=={pink}返回恢复现场==，回到之前被打断的位置=={pink}继续执行==。
...
大端小端描述的是**=={yellow}多字节数据在内存中的存放顺序==**。

常见在 通信协议、寄存器解析、跨平台数据传输 里考察。

- **底层实现原理**

1. **=={yellow}大端==**=={yellow}：高字节放低地址，低字节放高地址==，符合=={green}人阅读==数字的=={green}顺序==。
2. **=={yellow}小端==**=={yellow}：低字节放低地址，高字节放高地址==，很多=={yellow}{green} MCU/CPU 都采用==这种方式。
3. 比如 `0x12345678`，大端存放是 `12 34 56 78`，小端存放是 `78 56 34 12`。
4. 大小端=={pink}只影响==**=={pink}内存中的字节排列==**，=={pink}不影响==寄存器里看到的=={pink}数值本身==。
...
51 和 32 位单片机的区别，=={pink}本质是==**=={pink}老式 8 位 MCU 和现代 32 位 MCU 的能力差异==**。
...
32 位单片机一般通过 =={pink}PWM 改变电机两端的平均电压来实现调速==。

常见于风扇、电机、小车、电调、舵机控制。

- **底层实现原理**

1. STM32 用**=={yellow}定时器==**=={yellow}产生固定频率的 PWM 波，核心参数是 ==`PSC`==={yellow}、=={yellow}`ARR`=={yellow}、==`CCR`。
2. 其中 `ARR`=={yellow}{yellow} 决定周期，==`CCR`=={yellow} 决定高电平时间，占空比 = ==`CCR / ARR`。
3. 电机由于有电感和机械惯性，看到的不是快速翻转的高低电平，而是一个**平均电压**。
4. 占空比越大，平均电压越高，电机转速通常越快。
5. 所以 =={yellow}PWM 调速本质就是：==**=={yellow}频率基本固定，动态调整占空比==**。
...
它的好处是可以=={pink}像访问一个变量一样，直接访问寄存器中的某一位==。

- **底层实现原理**

1. Cortex‑M =={yellow}把一段 ====**={yellow}位带区**=={yellow}=={yellow} 的每一位，映射到 ====**={yellow}位带别名区**=={yellow}=={yellow}{yellow} 的一个 32 位地址==。
2. 这样=={green}访问别名区某个地址==，本质上就是在访=={green}问原地址中的某一位==。
3. =={green}对别名地址写 ==`0/1`==={green}，就等价于把目标位清 0 或置 1==；读别名地址，就能直接得到该位的值。
...
- =={pink}普通方式是 ====**={pink}按位运算改寄存器==**
- =={pink}位带方式是 ====**={pink}直接把这一位当变量访问==**

## 13 STM32 的 ADC 精度

- **应用场景**

=={yellow}ADC 精度决定了 ====**={yellow}模拟量转数字量时能分多细==**，直接影响电压、温度、电流等采样结果的分辨能力。

面试里一般既会问 **=={yellow}分辨率==**=={yellow}，也会追问 ====**={yellow}实际精度是否真的能达到标称值**=={yellow}=={yellow}。==

- **底层实现原理**

1. STM32 ADC=={yellow}{yellow} 常见分辨率有 ====**={yellow}12 位**=={yellow}=={yellow}，部分型号还支持 ====**={yellow}10/8/6 位==** 可配置，少数高端型号可到更高精度。
2. 以=={yellow}{yellow} ====**={yellow}12 位 ADC**=={yellow}=={yellow}{yellow} ==为例，表示=={yellow}可把输入电压范围分成 (2^{12}=4096) 个等级==，数字范围通常是 **0~4095**。
3. 若=={yellow}参考电压是 ==`3.3V`==={yellow}，则 1 LSB 约等于 =={yellow}`3.3 / 4096 ≈ 0.8mV`=={yellow}，这叫==**=={yellow}分辨率==**=={yellow}。==
...
=={yellow}时钟树用来给 ====**={yellow}CPU、总线、外设**=={yellow}=={yellow}{yellow} 分配不同频率的时钟。==

面试问它，本质是看你是否清楚=={yellow}{green} ====**={green}系统时钟从哪来、怎么分给各模块==。**

- **底层实现原理**

1. =={pink}STM32 的时钟源一般有 ====**={pink}HSI、HSE、LSI、LSE**=={yellow}，=={green}高速时钟给系统和外设==，=={green}低速时钟常给 RTC、看门狗==。
2. =={yellow}系统时钟 ==`SYSCLK`==={yellow} 可以选择直接来自 HSI/HSE，也可以先经过 =={yellow}**=={yellow}PLL 倍频**===={yellow}{yellow} 后再作为主时钟==。
3. `SYSCLK`=={yellow}{yellow} 再往下分成 ====**={yellow}AHB、APB1、APB2**=={yellow}=={yellow}{yellow} 等总线时钟==，分别供内核、内存、不同外设使用。
...
ARM 和 RISC‑V 的区别，本质是 **=={pink}成熟商业架构==**和 **=={pink}开源指令集架构==** 的区别。
...
5. 所以实际选型里，**=={pink}ARM 更偏成熟稳定，RISC‑V 更偏开放灵活==、成本和自主可控优势更明显。**
...
ISA = Instruction Set Architecture，指令集架构。它是硬件和软件之间的契约/接口。****

- 延申知识点
ISA、微架构、SoC 三者的区别

ISA 是软件看到的"指令语法"（如 ADD、MOV 怎么写）；微架构 是硬件怎么实现这些指令（流水线几级、缓存多大、乱序还是顺序）；SoC 是把 CPU 核 + 外设（GPIO、USB、DDR 控制器）封装成一颗芯片。- ARM Cortex‑M 和 RISC‑V MCU 在中断响应上的差异

## 18 交叉编译介绍一下，常见的交叉编译器

=={yellow}交叉编译 = 在 A 平台（如 x86 Linux PC）上编译出能在 B 平台（如 ARM 嵌入式设备）上运行的可执行代码。==
...
=={yellow}CPU默认从==`0x00000000`=={yellow}{yellow} 开始，但是从flash启动芯片会做一次地址重映射，从Flash的内存地址起点===={yellow}0x08000000===={yellow}开始取向量表.==
...
3. STM32=={yellow}{pink} 复位后虽然是从 ==`0x00000000`==={yellow}{pink} 开始取向量表，但芯片会把启动区域=={yellow}**=={pink}重映射**===={pink}到这个地址==。
...
- 启动时 STM32 会做一次**地址映射**，=={pink}让 CPU 能先取到 Flash 里的向量表==
- **延申知识点**
...
- **应用场景**
=={yellow}STM32 串口接收常见有 ====**={yellow}轮询、中断、DMA**=={yellow}=={yellow}{yellow} 三种方式。==
本质区别在于：**=={pink}CPU 参与程度不同，实时性和效率也不同==**。
- **底层实现原理**

1. **=={green}轮询==**=={green}：CPU 一直检查接收标志位 ==`RXNE`==={green}，有数据就读寄存器==；实现简单，但 CPU 会一直空转，效率最低。
2. **=={green}中断==**=={green}：收到数据后串口触发中断，CPU 跳到 ISR 里读 ==`DR`；比轮询省 CPU，适合低速或不连续数据。
3. **=={green}DMA==**=={green}：串口收到数据后，由 DMA 自动把 ==`DR`==={yellow}{green} 搬到内存==，CPU 不用逐字节参与；适合高速、大量、连续接收。
...
2. 配置入口本质都在=={yellow}{pink} ====**={pink}RCC 时钟控制模块==**，核心寄存器一般是 `RCC->CFGR`。
...
中速、不连续（如温度传感器每秒读几次）中断

## 23 STM32 Flash 怎么划分

- **应用场景**
STM32 的 Flash 划分主要关系到 程序存放、BootLoader/App 分区、参数掉电保存。
实际开发里经常会按区域划分成 **=={pink}代码区、升级区、参数区==**。
- **底层实现原理**

1. STM32 的=={yellow}{pink} Flash 是按 ====**={pink}页（Page）或扇区（Sector）**=={yellow}=={yellow}{pink} 管理==的，不是按字节独立擦除。
...
1. ADC 完成一次转换后，结果会先存到=={yellow}{pink} ===={pink}**ADC 数据寄存器 DR（Data Register，数据寄存器）**==。
2. =={green}如果是 ====**={green}轮询**=={yellow}=={green} 或 ====**={green}中断**=={yellow}=={green} 方式，CPU 再去读取 DR，读出来后一般存到变量、数组或缓冲区里==。
3. =={green}如果是 ====**={green}DMA**=={yellow}=={green} 方式，DMA 会自动把 DR 里的数据搬运到 RAM 中指定的数组或缓存区==。
4. 所以 ADC 数据最开始一定是先到 ADC 外设寄存器，之后才会进入 CPU 变量 或 内存缓冲区。
5. =={yellow}本质==上就是：`ADC -> DR寄存器 -> CPU读取或DMA搬到RAM`=={yellow}。==
...
1. **=={pink}中断一般是异步的==**，由=={yellow}外设事件触发（硬中断）==，比如串口接收、定时器溢出、GPIO 边沿。
2. **=={pink}异常一般是同步的==**，由=={green}当前执行指令触发（软件中断）==，比如除零、非法指令、访问非法地址。
...
1. 常用=={pink}在线调试接口是 ====**={pink}SWD 和 JTAG==**，STM32 最常用 SWD，占用引脚少。
...
SWD 是 JTAG 的精简版：线少一半、引脚省、功能够日常调试；JTAG 线多但支持边界扫描和多芯片链，FPGA 和复杂板级测试必须用。
...
1. =={yellow}外部中断常见触发方式==有**=={pink}上升沿、下降沿、双边沿、电平触发==**，STM32 EXTI 常用边沿触发。
...
考察 GPIO 输入稳定性，重点是=={pink}为什么输入引脚不能悬空==。

**底层实现原理**

1. GPIO 输入模式下，如果=={pink}外部没有明确高低电平，引脚会悬空，读到的值不稳定==。
2. =={green}上拉电阻把引脚默认拉到高电平，适合按键按下接地的场景==。
3. =={green}下拉电阻把引脚默认拉到低电平，适合按键按下接高电平的场景==。
4. 上拉 / 下拉=={yellow}提供默认状态==，=={yellow}避免==噪声导致=={yellow}误触发==。
...
3. 关键是=={pink}取出异常现场里的 ==`PC`==={pink}，它通常指向出错指令附近==。
...
=={yellow}发生异常进入中断 / 异常服务函数时，硬件自动压入栈的 8 个寄存器：==
...
> 
> 
> 通过 PC 反查代码：拿到 PC 值，结合 map 文件找函数，再看反汇编，映射到 C 源码行。
