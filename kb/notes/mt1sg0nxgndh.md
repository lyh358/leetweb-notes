...
2. 其中 `ARR`=={yellow} 决定周期，==`CCR`=={yellow} 决定高电平时间，占空比 = ==`CCR / ARR`。
...
1. Cortex‑M =={yellow}把一段 ==**=={yellow}位带区==**=={yellow} 的每一位，映射到 ==**=={yellow}位带别名区==**=={yellow} 的一个 32 位地址==。
...
2. 以=={yellow} ==**=={yellow}12 位 ADC==**=={yellow} ==为例，表示=={yellow}可把输入电压范围分成 (2^{12}=4096) 个等级==，数字范围通常是 **0~4095**。
...
=={yellow}时钟树用来给 ==**=={yellow}CPU、总线、外设==**=={yellow} 分配不同频率的时钟。==
...
=={yellow}CPU默认从==`0x00000000`=={yellow} 开始，但是从flash启动芯片会做一次地址重映射，从Flash的内存地址起点===={yellow}0x08000000===={yellow}开始取向量表.==
- **应用场景**
这个问题常用来考察你是否理解 STM32 的存储器映射和启动机制。
本质不是 “为什么偏偏这个数字”，而是 “这个地址是谁规定的”。
- **底层实现原理**

1. `0x08000000` 是 STM32 片内=={pink} Flash 在内存映射中的起始地址==，这是芯片硬件设计时就固定好的。
2. ARM 内核访问代码和数据，看到的是一张统一的**存储器地址映射表**，Flash、SRAM、外设都会被映射到不同地址段。
3. STM32=={pink} 复位后虽然是从 ==`0x00000000`=={pink} 开始取向量表，但芯片会把启动区域==**=={pink}重映射==**=={pink}到这个地址==。
...
- **应用场景**
=={yellow}STM32 串口接收常见有 ==**=={yellow}轮询、中断、DMA==**=={yellow} 三种方式。==
...
3. **=={green}DMA==**=={green}：串口收到数据后，由 DMA 自动把 ==`DR`=={green} 搬到内存==，CPU 不用逐字节参与；适合高速、大量、连续接收。
...
中速、不连续（如温度传感器每秒读几次）：中断高速、连续流（如音频 ADC、电机电流环）：
...
1. STM32 的=={pink} Flash 是按 ==**=={pink}页（Page）或扇区（Sector）==**=={pink} 管理==的，不是按字节独立擦除。
...
=={pink}
==|  | =={pink}原因== |  | =={pink}典型现象== |  | =={pink}核心原理== |  |
| --- | --- | --- | --- | --- | --- | --- |
|  | =={pink}电源不稳定== |  | =={pink}电机 / 外设启动时死机、复位，低电压场景下概率性跑飞== |  | =={pink}VDD/VCORE 纹波超标（STM32 要求 3.3V 纹波 < 100mV），电压跌落导致 MCU 寄存器乱码== |  |
|  | =={pink}晶振 / 时钟异常== |  | =={pink}系统时钟走偏、SysTick 延时错误，直接触发 HardFault== |  | =={pink}晶振停振 / 频率偏移，或 RCC 配置错误（如 HSI 不稳定却作为系统时钟）== |  |
|  | =={pink}虚焊 / 短路 / ESD 干扰== |  | =={pink}触碰电路板 / 工业环境下死机，PC 指针跳转到随机地址== |  | =={pink}虚焊导致引脚接触不良，ESD 冲击使寄存器 / 内存数据篡改== |  |
