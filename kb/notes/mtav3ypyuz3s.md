...
我在团队中担任**=={yellow}嵌入式终端负责人==**，负责智慧蜂箱设备端的**=={yellow}软硬件实现==**。**=={yellow}硬件方面==**，我完成了ESP32-S3、ESP32-CAM及温湿度、称重、GPS、OLED等=={yellow}模块的==**=={yellow}选型、接线和实物组装==**；**=={yellow}软件方面==**=={yellow}，我独立编写ESP32-S3主控端和ESP32-CAM图传端的程序==，并**=={yellow}**负责阿里云IoT平台的**设备注册、物模型定义和通信配置**。==
