...
=={green}更严重的是，冗余节点==**=={green}《打断了 ATC 算子融合的模式匹配==**——比如 `Conv→Unsqueeze→Shape→...→Add` =={green}中间夹着冗余节点**，ATC 识别不出**== `Conv+Add` =={green}**融合模式**，**只能将模型拆成 10+ 个子图**，子图间有大量**调度开销**和 **GM↔L1 数据搬运**。==
