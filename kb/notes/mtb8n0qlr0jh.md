...
> 
> 
> =={yellow}GPS侧，我将SoftwareSerial迁移到硬件UART==，把RX缓冲扩大到2048字节，并设置独立GPS任务每约10毫秒读取UART，再通过TinyGPSPlus进行流式解析。硬件UART负责底层位采样，较大缓冲吸收短时调度延迟，独立任务保证及时读取，三者一起降低高负载下的字节积压和NMEA解析失败风险。
> 
> 
