...
=={pink}编译时需要明确指定==**=={yellow}三组输入Shape==**=={yellow}、==**=={yellow}ND张量格式==**=={yellow}、==**=={yellow}目标芯片Ascend 310B4==**=={yellow}和==**=={yellow}精度模式==**。初版**采用FP32**，是因为回归任务精度优先。OM生成后，我还会在**=={yellow}板端通过ACL Lite编写推理程序==**=={yellow}实际==**=={yellow}加载和执行==**，而不是只以ATC编译成功作为部署完成的标准。
