...
1. =={yellow}**同一个数据结构**需要**维护**==`.proto`=={yellow}和==`.msg`===={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}**两份定义**；==
...
=={yellow}项目**为ROS的**==**`DataType`=={yellow}、==`MD5Sum`===={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}、=={yellow}`Definition`**=={yellow}**和**=={yellow}**`Serializer`**=={yellow}**等模板**增加了**Protobuf版本的偏特化。**==
...
=={green}这样，无论以后增加==`PublishInfo`=={green}、==`NodeMetrics`===={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={green}还是其他Protobuf消息，都**不需要为每种类型重新编写一套ROS适配代码**。==
...
=={yellow}在**编译期**（模板实例化阶段），对于某个**消息类型T**，T在尝试使用**实际类型**代替偏**特化模板的形参**的过程中（此时没有进入结构体内部代码），如果**产生了非法类型**/表达式，此时编译器**不报编译错误**，而是把这个**模板特化版本**从重载候选集里**丢弃**，编译器继续去**找其它可行版本**。==

如果**所有候选全部替换失败**，最后**才报编译错误**。

> ⚠️容易混淆的点：
> SFINAE 生效的是**参数替换阶段的错误**；如果模板已经选中某个特化，进入函数体内部代码报错，那就是普通编译错误，SFINAE 救不了。

=={pink}**SFINAE**与**全特化**和**偏特化**：==

1. **=={yellow}全特化==**：`template<> struct A<X>`
=={yellow}类型严格匹配，==**=={yellow}不涉及 SFINAE==**=={yellow}。对不上就直接回退主模板。==
2. =={yellow}带条件的**偏特化 /** 重载模板：===={yellow}**依靠 SFINAE**，**替换生成非法类型** →**丢弃该候选，继续其它匹配**。==
...
template<typename T>
struct IsMessage<T,typename enable_if<is_base_of_v<google::protobuf::Message, T>>::type
> : boost::true_type {};
```
...
`condition==is_base_ofgoogle::protobuf::Message,T::value`

```php
typename std::enable_if<std::is_base_of<google::protobuf::Message, T>::value>::type
```

如果**is_base_of::value是True**，**enable_if::type**返回**合法类型**。
...
- 如果类型 T 是 protobuf 的子类 → 使用我们写的那套 ROS Serializer /message_traits 逻辑
- 如果类型 T 是普通 ROS msg（gencpp 生成）→ 使用 ROS 原生模板逻辑
- 如果是别的普通类型 → 走默认主模板（`IsMessage=false`，禁止 publish）
...
1. **=={green}模板是编译期实例化==**=={green}，当 T 根本不是 protobuf 子类，==`dynamic_cast`=={green}这一行会**直接触发编译错误**，类型不兼容，**编译直接挂**，根本跑不到运行时。==
...
- =={yellow}Protobuf消息走**新增实现**；==
- =={yellow}ROS Msg走**原有实现**；==
- =={yellow}**两条路径互不影响；**==
- =={yellow}**不需要运行时**判断；==
...
=={green}ROS拿到一个C++类型后，**不会**通过大量**运行时判断**来分析它，而是**查询对应的Traits**，了解这个**类型具有什么性质**。==

例如：

| Trait | 回答的问题 |
| --- | --- |
| `IsMessage` | 这是不是一条**合法**的ROS消息 |
| `DataType` | 这条消息的**类型名称**是什么 |
| `MD5Sum` | **校验码**，通信双方使用的消息结构是否一致 |
...
> =={yellow}Traits可以理解为ROS的**编译期类型档案**。ROS**通过Traits判断**一个**类型是不是消息**==、类型名称是什么、长度是否固定、有没有Header，以及通信双方是否兼容。项目中我=={yellow}通过**偏特化**为所有**Protobuf**类型**补充了这些信息**，让ROS能够**把Protobuf类当作合法消息**处理==。
...
=={green}如果只补充Traits而不补充Serializer，ROS**虽然知道它是一条消息**，但仍然不**知道怎样实际传输它。**==
...
=={green}调用**Protobuf提供的序列化接口**，将**消息转换成二进制字符串**===={green}：==
...
=={green}长度字段用于告诉接收端，**后面应该读取多少字节**作为一个完整的Protobuf消息体。==

#### =={yellow}第三步：ROS完成传输==

=={green}Serializer**只负责**将**对象转换成ROS能够发送的字节流**==。后面的连接管理、发送队列和TCPROS传输仍然由ROS负责。

因此，=={green}项目没有重新实现网络通信协议==。
...
- 自己实现容易产生协议兼容和边界处理问题；
- =={yellow}项目的重点是ROS与Protobuf的**适配**==，而不是重新设计序列化协议。
...
> =={yellow}**模板偏特化**提供**扩展入口**，**SFINAE**负责在编**译期筛选Protobuf类型**，**Traits**解决ROS对消息的**类型识别**问题，**Serializer**解决消息的**二进制收发问题**。==
...
> 
> 
> 这样，=={green}上层节点不需要编写Protobuf到ROS Msg的转换代码，可以直接使用ROS原有的==`publish`=={green}和==`subscribe`===={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={green}接口传输Protobuf消息，同时原生ROS Msg也不会受到影响。==
...
=={yellow}因此，**每个计算节点都需要一个Monitor Agent**，**周期性采集本机**的==：
...
`/proc`=={yellow}是Linux提供的一个==**=={yellow}虚拟文件系统==**=={yellow}。==

=={green}它看起来像普通文件目录==，但其中大部分内容并不真正保存在硬盘上，而是由Linux内核根据当前运行状态动态生成。

可以把它=={pink}**理解**成==：

> =={yellow}Linux**内核对用户程序开放的**一组“**系统状态查询接口**”。==
...
`/proc/stat`记录的不是当前CPU利用率，而是=={green}CPU从系统启动以来，**在用户态、内核态、空闲等状态下累计运行的时间**。==

因此，不能只读取一次，而是要=={yellow}比较**前后两次采样**==：
...
因此，`NetworkCollector`保存**=={yellow}上一次的收发字节数===={yellow}**和==**===={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}={yellow}时间戳=={yellow}=={yellow}**，通过两次采样的**===={yellow}字节差除以时间差==**=={yellow}，得到==**=={yellow}每秒收发字节数==**=={yellow}。==
...
- Agent同时承担采集、计算和通信职责；
- CPU、内存和网络逻辑**=={yellow}耦合==**在一起；
- 增加或关闭某一类指标时需要修改Agent主流程；
- 后续增加磁盘、温度或GPU指标时，代码越来越庞大。

因此，我将=={yellow}**每一类指标**封装成**独立采集器**，并让所有采集器遵循**同一个接口**==：
...
这个=={pink}接口规定**所有采集器**都必须**具备两项能力**===={pink}：==

- `Collect()`：=={yellow}**采集指标并写入**==`NodeMetrics`；
- `Name()`：=={yellow}**返回采集器名称**==。

=={pink}在此基础上实现：==

=={yellow}采集器接口**基类**（虚函数采集接口函数），**继承与多态**实现具体的**采集子类**函数==
...
继承用于=={yellow}建立统一**接口**与**具体采集器**==之间的关系。
...
> =={yellow}使用**同一个基类接口**，实际执行**不同子类的实现（4类采集器）**。==

=={pink}Monitor Agent**不需要分别编写4**个==：
...
=={yellow}而是将**所有采集器**用**智能指针unique_ptr**保存到**同一个vector容器**中==：
...
> =={yellow}Agent**只依赖**==`IMetricCollector`=={yellow}**基类**，并**通过虚函数**调用**不同子类的**==`Collect()`。这样Agent=={green}不需要关心采集器的具体类型，**遍历一个容器**就能**完成所有指标采集**==。
...
因此，=={yellow}项目使用==**=={yellow}unique_ptr管理采集器==**：
...
- =={green}Agent**统一拥有**这些采集器==；
- =={green}不允许随意复制采集器对象==；
- =={green}Agent**退出时自动释放**所有采集器==；
- 避免裸指针产生内存泄漏和所有权不清的问题。

=={yellow}**工厂创建采集器后**，通过==`move`=={yellow}将**所有权转移给Agent的容器**。==
...
**项目里的工厂类**：`CollectorFactory`**采集器有一堆子类**：`CpuCollector`、`MemoryCollector`、`NetworkCollector`、`IrqLoadCollector`，全部继承抽象基类 `IMetricCollector`。
