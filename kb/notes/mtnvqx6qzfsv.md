...
> 互斥锁**=={yellow}本质==**是一个**=={yellow}内核结构体==**，**=={green}内部包含==**：**=={yellow}1.锁计数、2.持有该锁的任务句柄、3.优先级继承相关的成员。==**
...
> 1. 互斥锁有任务所有权，只有持有者可以释放；二值信号量没有所有者，任意任务都可以 give。
> 2. 互斥锁自带优先级继承机制，专门用来解决优先级反转，用于资源互斥保护；二值信号量没有优先级继承，多用于事件通知。
> 3. 互斥锁支持递归版本；二值信号量不支持递归。
