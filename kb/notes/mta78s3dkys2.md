...
1. =={yellow}SPF模型本身计算量不大==；
2. =={yellow}Conv和MatMul虽然运算量较高，但Cube处理矩阵计算的吞吐能力较强；==
3. GELU、Softmax和LayerNorm等=={yellow}Vector计算的理论效率相对较低==；
4. =={yellow}理论计算时间只计算**理想算术开销**，**没有**完整包含调度、搬运和格式转换。==
